...
**Date:** 2026-05-19
**Scope:** `docs/` tree — iVisit app docs reorganization
...
## Files Left in Place — Marked Historical (Notice Added)
...
| `docs/audit/emergency/EMERGENCY_FLOW_FULL_CYCLE_AUDIT_2026-04-24.md` | Completed audit; issues addressed in tracking sheet passes A–G. Useful root-cause record. | `TRACKING_STATE_TIGHTENING_PASS_2026-05-19.md` |
...
| `docs/audit/emergency/EMERGENCY_CONTEXT_MODULARIZATION_PLAN.md` | Plan is complete — EmergencyContext.jsx fully retired (Phase 5, sub-passes 5a–5f). | `GOLD_STANDARD_STATE_ROADMAP.md` |
...
| `docs/audit/planning/UNIFIED_MODULARIZATION_PASS_PLAN.md` | Pass plan complete. MapScreen passes 0–2 done. | `docs/audit/checkpoints/FINAL_MIGRATION_SUMMARY.md` |

---

## Current Recommended Reading Path — Emergency Flow Work
...
6. **[docs/audit/map/TRACKING_SHEET_PHASE_AUDIT_2026-04-26.md](map/TRACKING_SHEET_PHASE_AUDIT_2026-04-26.md)**
   Full tracking sheet audit (passes A–G complete). Phase contracts, Apple HIG polish, defect classes.

### Supporting reads

- **[docs/architecture/state/GOLD_STANDARD_STATE_ROADMAP.md](../architecture/state/GOLD_STANDARD_STATE_ROADMAP.md)** — Five-layer architecture phases 1–7, completion record.
- **[docs/architecture/refactoring/TRACKING_SHEET_LEARNINGS.md](../architecture/refactoring/TRACKING_SHEET_LEARNINGS.md)** — Defect classes 2.1–2.14, heuristics H1–H5, sweep targets.
- **[docs/audit/map/PRE_TRACKING_PHASE_AUDIT_2026-04-27_FINAL.md](map/PRE_TRACKING_PHASE_AUDIT_2026-04-27_FINAL.md)** — Pre-tracking gate requirements.
...
- `MAP_FLOW_IMPLEMENTATION_V1.md` → moved to archive; replaced by `MAP_SCREEN_IMPLEMENTATION_RULES_V1.md`
- `MAP_EXPLORE_INTENT_AUDIT_AND_SYSTEM_V1.md` → moved to archive; replaced by Explore Care dossier + EXP passes
- `EMERGENCY_SHEET_AND_MAP_UI_SPEC_V1.md` → moved to archive; replaced by tracking audit + implementation rules
- `MAP_RUNTIME_PASS_PLAN_V1.md` → moved to archive; replaced by Gold Standard roadmap
- `EMERGENCY_CONTEXT_MODULARIZATION_PLAN.md` → noticed in-place; EmergencyContext retired

### Overlapping but distinct — left in place

These pairs cover overlapping ground but contain distinct enough detail to keep:

| Pair | Distinction |
|---|---|
| `MAP_FLOW_FINAL_POLISH_AUDIT_2026-04-20.md` vs `TRACKING_SHEET_PHASE_AUDIT_2026-04-26.md` | The 04-20 audit covers the full flow; the 04-26 audit is tracking-specific with passes A–G. Different scopes. |
...
| `EMERGENCY_FLOW_FULL_CYCLE_AUDIT_2026-04-24.md` vs `EMERGENCY_FLOW_STATE_SYNC_AUDIT_2026-04-24.md` | Different angles of the same date — full cycle vs animation/state sync specifically. Both marked historical. |
...
| `docs/project_state/context/SUPABASE_CHANGE_CONTROL_PLAN_2026-03-05.md` | 87 KB — the largest single doc. Active change control or historical? |
| `docs/project_state/context/SUPABASE_CHANGE_TRACKER_2026-03-05.md` | 109 KB — the largest single doc. Active tracker or frozen? |
| `docs/project_state/context/` — SCC-001 through SCC-058 | 57 contract/hardening docs from 2026-03-05. Are any still tracking open items, or are they all closed? |
...
- All `docs/flows/emergency/architecture/` sub-pass docs (EXP, LOC, contact-dispatch passes — active work in progress)
- All `docs/architecture/ux/passes/` (UX-A through UX-E — PLANNED/ACTIVE)
- All `docs/audit/demo/` passes (PLANNED — active work)
