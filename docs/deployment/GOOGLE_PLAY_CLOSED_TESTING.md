...
1. Bump `version` in `app.config.js` (e.g. `1.0.4` â†’ `1.0.5`).
   - **This is the `runtimeVersion`.** `eas.json` policy is `appVersion`, so the runtime version equals `version`. Without bumping this, old OTAs and new OTAs share the same runtime key and Expo will serve whichever matches its own SDK fingerprint â€” usually the old one.
...
4. Update `ivisit/src/constants/appLinks.ts` â€” both `ANDROID_PREVIEW_UPDATE_URL` and `IOS_PREVIEW_UPDATE_URL` â€” with:
   ```
...
### After a full rebuild â€” publish matching OTA
...
```bash
eas update --channel staging --message "OTA vX.Y.Z â€” clean runtime for staging build versionCode <N>"
```

Then update `appLinks.ts` with the new group ID (same as OTA-only step 4â€“5 above).
...
1. **`runtimeVersion` was never bumped.** `app.config.js` `version` stayed at `1.0.4` across every build and OTA for months. All updates â€” old and new â€” shared the same runtime key `1.0.4`. Expo Go matched the OTA whose fingerprint fit its own SDK, which was the oldest compatible one.
...
3. **`PreviewBridge.tsx` was hardcoded to `'production'`** â€” `getAppDownloadLinks('production')` â€” so the "Open iVisit" button in the modal was always sending users to the production `exp://` link regardless of `appLinks.ts` updates.

**Fixes applied:**

- Bumped `version` to `1.0.5` in `app.config.js` â€” new clean runtime key.
- Built staging again (versionCode `23`, build `b631dbc0`).
- Published OTA **after** the build finished â€” group `239408e5`, runtime `1.0.5`.
...
2. **Always publish the OTA *after* the native build completes**, not before â€” so both share the same fingerprint.
3. **Always update `appLinks.ts` immediately after publishing the OTA** â€” do not wait.
