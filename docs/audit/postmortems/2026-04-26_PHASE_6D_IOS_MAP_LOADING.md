...
> **Severity**: Critical â€” full app unusable on iOS for several hours.
> **Date**: 2026-04-26
...
- iOS only â€” Android worked throughout.
- Map screen stuck on `"Loading map / Live surface"` overlay forever.
- Even after overlay dismissal experiment, map showed only the static `loadingBackgroundImageUri` â€” no live tiles.
...
1. **First hypothesis** â€” overlay state machine bug. Added a session latch in `useMapLoadingState`. Helped overlay dismiss but underlying `mapReadiness.mapReady` was never propagating.
2. **Second hypothesis** â€” store re-render not firing. Added `[MapCB]` log. Confirmed `setMapReadiness` IS dispatching and the parent IS re-rendering with new `mapReadiness`. So why wasn't `useMapLoadingState`'s effect firing?
3. **Third hypothesis** â€” native map module broken. Added a bare `<MapView>` directly in `MapScreen`. It rendered tiles and fired `onMapReady` â€” proving `react-native-maps` was fine.
4. **Fourth hypothesis (WRONG â€” noted for honesty)** â€” suspected `mapType="mutedStandard"` was silently failing on iOS dev clients. Switched to `"standard"` and observed tiles loading. **This was a misattribution.** The map was actually rendering all along; the loading overlay was just covering it. The latch and validity fixes I added in parallel were what actually unblocked rendering. `mutedStandard` is a valid Apple Maps style with no entitlement requirements. Decision: keep `"standard"` for now since it's the simpler default, but `mutedStandard` can be used freely in iOS dev or prod.
5. **Real cause finally surfaced** via `[MapLoad] render` log: `hasActiveLocation` was flipping from `true` â†’ `false` between renders, **even though `[MapLoc] sources` reported `activeLocation: "set"`**. The `||` chain was picking up a truthy-but-empty `emergencyUserLocation` object from the Zustand store.
...
1. **`useMapLocation` `||` chain accepted truthy-but-coordinate-less objects.** Phase 6d migrated `userLocation` from local `useState` â†’ Zustand `useLocationStore`. The store could hold a partial/empty object that was truthy but had no valid `latitude`/`longitude`. The `||` chain `manualLocation?.location || emergencyUserLocation || globalUserLocation` picked the empty store object and short-circuited â€” masking `globalUserLocation`'s real coordinates. `hasActiveLocation = Boolean(lat && lng)` then evaluated `false`, blocking `isMapFrameReady` forever.
...
| `components/emergency/intake/EmergencyLocationPreviewMap.jsx` | Removed `!hasLocation` guard from `isMapReady` fallback timeout so it always fires. (Switched `mapType` to `"standard"` during debugging â€” retained as a simpler default, but `mutedStandard` is also valid; this was not a real fix.) |
...
3. **Don't assert hypotheses as root causes without isolated verification.** During this session I claimed `mutedStandard` was the bug because the symptom went away when I changed it â€” but I had also changed two other things. Always isolate one variable at a time before declaring root cause.
4. **Auth guards in route layouts must not rely on `loading` alone.** Always seed from cache before async network sync, so `loading=false` implies authoritative state. The `loadingâ†’false` transition is a contract, not a hint.
...
|---|---|---|
| `modeStore.js` | Primitives only (`string \| null`) | âœ… Safe |
| `coverageStore.js` | `coverageModeOperation` always written with full shape | âœ… Safe |
| `emergencyTripStore.js` | Trip objects always carry `requestId`; consumers gate on it | âœ… Contained |
| `locationStore.js` | **Was vulnerable** | âœ… Now hardened (validity guard added) |
...
- [ ] Optional: re-enable `mutedStandard` on iOS for the muted Apple Maps look â€” fully supported, no entitlements required.

## Related Documents

- Originating phase: `architecture/state/GOLD_STANDARD_STATE_ROADMAP.md` Â§ Phase 6d
