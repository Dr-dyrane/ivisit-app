...
GPS drift of 10â€“50 m between sessions produces a new scope key â†’ a new org â†’ a new set of 5â€“6 hospitals written permanently alongside the previous set.
...
// Changed to ctx.userSlug â€” stable across all sessions and all cities.
...
- Derived from the Supabase auth user ID â€” stable across all sessions and all cities
- `toSafeUserSlug` already strips non-alphanumeric chars and clamps to 12 chars â€” safe for email and `place_id` embedding
...
| slot cleanup prefix | `demo:<userSlug>:slot:` â€” correct prefix for stale slot sweep |
...
| `toCoverageKey` function | Still used to populate `ctx.coverageKey` â€” used only for client display/logging, not scope identity |
| `place_id LIKE 'demo:%'` prefix | Unchanged â€” all scripts, markers, and queries depend on this |
| `DEMO_MIN_HOSPITALS = 5` / `DEMO_MAX_HOSPITALS = 6` | Unchanged |
| `nearby_hospitals` RPC | Unchanged |
| `isDemoSeedRow` filter | Unchanged â€” still filters out demo rows from seed input |
| `upsert on place_id` conflict resolution | Unchanged â€” still correct, now benefits from stable keys |
| `status = 'full'` retirement pattern | Unchanged â€” active pool rule still applies |
...
`toSafeUserSlug` falls back to `"guestdemo"` when the normalized value is empty (line 352â€“354).
All guest users share the same slug and therefore the same org.
**This is acceptable and intentional** â€” guest sessions always shared one provisioning ID via `resolveProvisioningUserId` on the client. No regression.
...
UUID collision after 12-char alphanumeric truncation is extremely unlikely (UUIDs contain hex chars only â€” `a-f0-9` â€” so collisions require first 12 hex chars to match).
**Risk: negligible.** If a collision occurs, two users share one org â€” same as the old guest behavior. Not a safety issue for demo mode.
...
No â€” `findCityDemoFallbackCatalog` only reads `ctx.latitude/longitude` to check distance from catalog reference points. It does not call `resolveDemoSeedScopeKey`. No circular dependency.
...
```
After the change, prefix becomes `demo:<userSlug>:slot:`. This correctly targets only the current user's slot-based hospitals. Old coordinate-scoped slot rows (`demo:p4365:slot:1`) are NOT cleaned by this prefix â€” they are handled by the cross-org sweep in Pass 2.
...
```
Cross-check: `matchesDemoScope` or similar function that reads this feature â€” confirm in `demoEcosystemService.js` and `hospitalIdentity.js` before closing this pass.
...
Pass 5 is the doc update â€” must ship in the same commit as Pass 1 + 2.
...
|---|---|---|
| `toCoverageKey` | Yes â€” used to set `ctx.coverageKey` at line 1951 | Safe â€” ctx field still exists for logging |
| `toCoverageAxisKey` | Yes â€” called by `toCoverageKey` | Safe |
| `findCityDemoFallbackCatalog` | Yes â€” called in `getCatalogSeedHospitals`, `ensureDemoHospitals` | Safe |
| `getCatalogSeedHospitals` | Yes â€” still returns hospital templates | Safe |
| `resolveDemoSeedScopeKey` | Yes â€” one-liner change only | Safe |
...
- [ ] Two bootstrap calls from coords 200 m apart for the same user â†’ same org ID
- [ ] `place_id` values for new hospitals all share `demo:<userSlug>:` prefix
- [ ] Hospital count per user stays at 5â€“6
