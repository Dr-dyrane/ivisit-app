# Pass 3 — DB: Retire Existing Duplicate Demo Hospitals

**Track:** Database / Scripts
**Date:** 2026-05-10
**Status:** PLANNED — gated on Pass 1 + 2 staging verification
...
The Toronto user's issue (and similar users worldwide) shows rows that are already in the DB: multiple sets of `demo:p<coords>:…` hospitals sitting `status = available` from prior coordinate-scoped bootstrap runs. These appear on the map and list immediately, regardless of what the new code does.
...
Follows the canonical sequence from `docs/flows/emergency/DEMO_MODE_COVERAGE_FLOW.md §Demo Cleanup Runbook`:

```
1. node supabase/scripts/dedupe_demo_hospitals.js           (dry run — review output)
2. SQL coordinate-cluster SELECT                             (dry run — review candidates)
...
5. node supabase/scripts/cleanup_demo_orphans.js            (dry run — confirm no orphan drift)
...
-- Groups by name (first 40 chars normalized) + 0.003° bucket (~330m)
...
- Are any `place_id` values using the new `demo:<userSlug>:…` format? (If yes, Pass 1+2 were already deployed — confirm before retiring)
- Row count — if >100 rows, consider batching the UPDATE
...
```sql
-- STEP 2: APPLY — retire duplicate demo hospitals
-- Only run after reviewing the dry-run SELECT above
-- Status = 'full' is the only mutation — consistent with active pool rule and cleanup runbook
...
### EC-1: Coordinate bucket (0.003°) clusters hospitals that are NOT duplicates
0.003° ≈ 330 m. Two distinct hospitals 200 m apart with different names will NOT be clustered (different `name_norm`). Two distinct hospitals 200 m apart with identical names (uncommon but possible) WILL be clustered — one will be retired.
...
### EC-3: Rows with `place_id` using new `demo:<userSlug>:…` format are in the candidate set
This means Pass 1+2 were already deployed before Pass 3. The new-format rows are the keepers — the coordinate-format rows should be the ones retired.
...
Per `DEMO_MODE_COVERAGE_FLOW.md §Acceptance Checks` and the bloat remediation checkpoint:
...
| `demo:p<coords>:…` rows with `status = available` | 0 |

---

## Verification Checklist

- [ ] `dedupe_demo_hospitals.js` dry run reviewed — output makes sense
- [ ] SQL coordinate-cluster SELECT dry run reviewed — no false positives
- [ ] `dedupe_demo_hospitals.js --apply` run on staging
- [ ] SQL UPDATE applied on staging
- [ ] `cleanup_demo_orphans.js` dry run on staging — zero orphan drift
- [ ] `cleanup_demo_orphans.js --apply` run on staging
- [ ] Active demo hospital count per user = 5–6 after cleanup
- [ ] Zero `demo:p<coords>:…` rows with `status = available` remain
