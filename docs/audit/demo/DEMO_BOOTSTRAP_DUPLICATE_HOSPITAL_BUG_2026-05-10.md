...
**Severity:** High — visible to all demo-mode users worldwide; causes map/list clutter, erodes trust.
...
- `docs/flows/emergency/DEMO_MODE_COVERAGE_FLOW.md` — the canonical demo rules source
- `docs/audit/map/DEMO_BOOTSTRAP_BLOAT_REMEDIATION_CHECKPOINT_2026-05-01.md` — prior remediation and cleanup sequence
- `docs/architecture/overview/ARCHITECTURE.md` — three-layer separation of concerns
- `docs/architecture/refactoring/REFACTORING_BIBLE.md` — PULLBACK NOTE convention, pass discipline
- `docs/architecture/state/GOLD_STANDARD_STATE_ROADMAP.md` — five-layer state ownership
- `docs/audit/checkpoints/STACK_GUARDRAIL_RECONCILIATION_CHECKPOINT_2026-04-29.md` — useEffect guardrails
- `docs/audit/map/NEAREST_HOSPITAL_SELECTION_AUDIT_2026-05-07.md` — nearby lane discipline
- `docs/audit/map/MAP_LOCATION_NEARBY_AND_ROUTE_FAILURE_AUDIT_2026-05-07.md` — location truth contract
- `docs/audit/map/search/LOCATION_ARCHITECTURE_AUDIT_2026-05-08.md` — 5-layer location system
- `docs/architecture/ux/IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS_2026-05-10.md` — no auto-commit, explicit CTAs
- `docs/architecture/ux/APP_WIDE_SURFACE_AUDIT_FOR_LOCATION_2026-05-10.md` — surface reuse rules

### Key prior decisions that constrain this pass

| Rule | Source | Constraint on this fix |
|---|---|---|
| Active demo pool = `status = available` only | DEMO_COVERAGE_FLOW §Active Demo Pool Rule | Retirement = `status = full`, never DELETE |
| Cleanup runs in order: dedupe → orphans | DEMO_COVERAGE_FLOW §Cleanup Runbook | Fix must be script-compatible with existing tools |
| Metro catalog replaces per-bucket rows; legacy rows must be retired | DEMO_COVERAGE_FLOW §Metro Catalog Rule | Scope fix must not re-introduce per-bucket proliferation |
| `5–6 active hospitals` per scope | DEMO_COVERAGE_FLOW §Demo Bootstrap Volume Rule | Scope fix must not allow count to grow beyond 6 |
| Bootstrap is idempotent and upsert-safe | DEMO_COVERAGE_FLOW §Failure Handling | place_id stability is essential; cannot change format unpredictably |
| PULLBACK NOTE on every changed line | REFACTORING_BIBLE §Commandment 8 | All code changes must carry inline reversal comments |
| Carry-forward: update `DEMO_MODE_COVERAGE_FLOW.md` in the same pass | BLOAT_REMEDIATION checkpoint §Carry-Forward Rule | This doc must be updated alongside code |
| `nearestHospital` derived from strict `≤5km` lane | NEAREST_HOSPITAL_SELECTION_AUDIT | Fix must not widen nearby lane beyond 5km for hospital markers |
| No `useEffect` to mirror state | STACK_GUARDRAIL_RECONCILIATION | No new effect-driven state sync on client |
| Location is the canonical owner of billing country | PICKUP_CONTROL_AND_QUOTE_ADOPTION_AUDIT | Demo scope change must not break location→billing chain |

---

## 1. Root Cause (Confirmed)

The scope key driving org identity and `place_id` generation embeds the user's **coordinates** at 1.1 km precision. GPS drift of 10–50 m between sessions produces a new scope → new org → new 5–6 hospitals written permanently alongside the previous set.

**Toronto** has no city catalog entry — only Lagos is in `CITY_DEMO_FALLBACK_CATALOGS`. Every Toronto user runs on the raw coordinate scope path with zero proliferation protection.

The correct fix: **scope = user slug, not location.**
- One org per user, stable forever across all cities
- `place_id` format: `demo:<userSlug>:src:<sourceId>` — stable per user+source
...
1. **`dedupe_demo_hospitals.js` and `cleanup_demo_orphans.js`** — scripts remain valid; retirement = `status = full`
2. **Active pool rule** — only `status = available` rows count; fix must not re-activate retired rows
3. **Metro catalog seed data** — `CITY_DEMO_FALLBACK_CATALOGS` stays as a hospital seed source; only its scope-key override is removed
4. **5–6 hospital cap** — `DEMO_MIN_HOSPITALS = 5`, `DEMO_MAX_HOSPITALS = 6` unchanged
5. **`nearby_hospitals` RPC** — fix must not alter the discovery or nearby lane used by `/map`
6. **`isDemoSeedRow` filter** — `getNearbySeedHospitals` already filters demo rows from seeds; this must keep working
7. **Hospital marker render contract** — no change to `place_id LIKE 'demo:%'` prefix; markers, queries, and scripts all depend on this prefix
8. **`demo_owner:` feature flag** — still used by `matchesDemoOwner`; the feature tag format does not change
9. **Acceptance checks 21 & 22 in DEMO_COVERAGE_FLOW** — repeated bootstrap must not grow active count; historical `full` hospitals must not generate new staff
...
**Risk:** Low — contained inside the function; all downstream logic uses the scope key value, not its derivation
...
// all cities worldwide — prevents per-session org proliferation from GPS drift.
...
`ctx.userSlug` is already on `DemoContext` (line 306) and populated at the call site (line 1950) via `toSafeUserSlug(effectiveUserId)`. It is derived from the Supabase auth user ID — stable across all sessions and all locations.
...
`ensureDemoOrganization` keys by `contact_email = demo+coverage-<scopeKey>@ivisit-demo.local`. After this change it becomes `demo+coverage-<userSlug>@ivisit-demo.local`. No code change needed — the value flows from `resolveDemoSeedScopeKey`. The lookup still uses `.maybeSingle()` (line 1116 — confirmed correct).

### Verification
- Deploy to staging
- Bootstrap from two GPS coordinates 200 m apart for the same user
- Confirm: same org ID returned both times
- Confirm: hospital count stays at 5–6, not 10–12
...
**Risk:** Low — sweep only retires, never deletes; mirrors the active pool rule already in place
...
// Radius = 0.15° (~16km) — wider than DEMO_HOSPITAL_OFFSETS (~1km) but
...
**Why this is safe:** After Pass 1, each user has exactly one org. A cross-org demo hospital in the same area must belong to an old coordinate-scoped org from a prior bootstrap run. Retiring it is correct — it was already semantically stale.
...
- Rerun `cleanup_demo_orphans.js` — confirm no regressions in orphan counts

---

## Pass 3 — DB Cleanup: Retire Existing Duplicates
...
**Risk:** Medium — must be gated on dry-run confirmation before applying
...
```sql
-- Step 1: DRY RUN — identify duplicate demo hospital clusters
-- Group by name + 0.003° coordinate bucket (~330m)
WITH clustered AS (
  SELECT
    id,
    place_id,
...
1. `node supabase/scripts/dedupe_demo_hospitals.js` — dry run first
2. Run the SQL dry-run SELECT above on staging
3. Review candidate rows — confirm no false positives
4. `node supabase/scripts/dedupe_demo_hospitals.js --apply` OR apply SQL UPDATE
5. `node supabase/scripts/cleanup_demo_orphans.js` — dry run to confirm zero orphan drift
...
- Active demo hospitals per user scope: 5–6
...
**Effort:** ~15 lines
**Risk:** Low — gate becomes stricter, not looser; worst case is an extra bootstrap call

Currently `getPersistedDemoCoverageForLocation` counts demo hospitals from **any org** within the radius. After Pass 1, a different user's org could have 5+ hospitals at the same location. The coverage gate would return `sufficient: true` and skip bootstrap — leaving the current user with no org and no hospitals they can dispatch.
...
`matchesDemoOwner` already exists (line ~208 of `demoEcosystemService.js`) — no new logic needed.
...
## Pass 5 — Doc: Update `DEMO_MODE_COVERAGE_FLOW.md`
...
Add a new section after `§Active Demo Pool Rule`:
...
- This rule applies worldwide — no per-city catalog entry is required.
...
```
Pass 1 + 2  →  deploy Edge Function to staging
            →  verify same-user org stability (two coords, same org)
            →  verify cross-org sweep retires stale rows
Pass 3      →  dry-run SQL on staging → review → apply on staging
            →  cleanup_demo_orphans.js dry run → confirm zero orphan drift
            →  apply on production in maintenance window
Pass 4      →  client PR, staging test with two users at same location
Pass 5      →  update DEMO_MODE_COVERAGE_FLOW.md in same commit as Pass 1+2
```
...
Pass 4 is a separate client PR — system is correct without it; gate just becomes stricter.
...
| File | Pass | Change |
|---|---|---|
| `supabase/functions/bootstrap-demo-ecosystem/index.ts` | 1, 2 | `resolveDemoSeedScopeKey` → userSlug; cross-org sweep |
...
- Never delete demo hospital rows — always `status = 'full'`
- Never add a unique constraint on `(latitude, longitude)`
- `place_id LIKE 'demo:%'` prefix stays — all scripts, queries, and markers depend on it
- `CITY_DEMO_FALLBACK_CATALOGS` stays — seed data, not scope control
- `DEMO_MIN_HOSPITALS = 5`, `DEMO_MAX_HOSPITALS = 6` unchanged
- `nearby_hospitals` RPC and the strict `≤5km` nearby lane are untouched
