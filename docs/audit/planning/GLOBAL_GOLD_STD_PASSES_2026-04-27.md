...
**Guardrails**: `TRACKING_SHEET_LEARNINGS.md` defect classes 2.1–2.16 Â· 5-layer architecture Â· PULLBACK NOTE convention Â· DEPRECATED_FILE export standard
...
| `EmergencyLocationSearchSheet` | â„¹ï¸ Legacy island | No live consumer — entirely in deprecated cluster |
...
6. **Fix #5** — `useMapSearchSheetModel`: `activeMode` — â›” CANCELLED: `activeMode` is intentionally reset to `mode` prop on every `visible` change (lines 67, 88). Converting to atom would override caller's `mode` prop on re-open — regression. Correct as `useState`.
