...
Ambulance sprites on Android native build render at incorrect size (128×128 - the raw PNG bitmap dimension) instead of the intended design size (~46×46 or ~90×90).
...
Current ambulance sprites are 128×128 (from `generate-ambulance-sprites.ps1` CanvasSize=128), but code specifies `imageSize: 46×46` which only works on web.
...
- Normal: 28×48 (web) → 54×91 (native) = ~1.93x width, 1.9x height
- Selected: 38×64 (web) → 68×114 (native) = ~1.79x width, 1.78x height
- **Average ratio: ~1.9x**

Applied to ambulance sprite (46×46 web):
- **Native target: 46 × 1.96 = ~90×90**

### Implementation Strategy

1. **Code changes** (immediate): Define 90×90 native size in `imageSize`
2. **Asset regeneration** (required for native): Resize PNGs from 128×128 → 90×90
...
|----------|------|-------|
| Web | 46×46 | 1x (base) |
| Native (iOS/Android) | 90×90 | 1.96x (matches hospital ~1.9x) |

**Note**: Current PNGs are 128×128. Native builds will show oversized sprites until PNGs are regenerated at 90×90.
...
The ambulance sprites must be regenerated at 90×90 instead of 128×128:
...
This will regenerate all 16 ambulance heading sprites (ambulance_00.png through ambulance_15.png) at the correct 90×90 size.

## Verification

- [ ] Web: Ambulance sprite renders at 46×46
- [ ] Android native: Ambulance sprite renders at 90×90 (after PNG regeneration)
- [ ] iOS native: Ambulance sprite renders at 90×90 (after PNG regeneration)
