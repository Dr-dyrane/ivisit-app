...
**Guide**: `TRACKING_SHEET_LEARNINGS.md` defect classes 2.1–2.14 Â· `STASH_AUDIT.md` pending items.
...
**Gold Standard fix**: Adopt `usePaymentMethodsQuery.ts` + `usePaymentCostCalculation.ts` from stash (â³ PENDING in STASH_AUDIT.md). TanStack Query caches independently — cost query completing does not re-trigger the methods query. Pass in `totalCostValue` to the methods query as `enabled: Boolean(totalCostValue)` or as a stable `queryKey` param — it becomes a cache key, not a reactive dep that causes refetch. **Pass: PT-B**.
...
**5-layer question (H1)**: Is `commitFlow` persistent client state? No — it is a UI wizard breadcrumb that only needs to survive within a session (sheet navigation, background/foreground). It belongs in Jotai (layer 5). The stash has `atoms/commitAtoms.ts` (â³ PENDING).
...
`refreshPaymentMethodSnapshot` always calls `getPaymentMethods()` + `getWalletBalance()` + `database.read(DEFAULT_PAYMENT_METHOD)` in parallel. In an emergency flow, a user scanning payment options (wallet → cash → card) fires 3 parallel network calls Ã— 3 taps = 9 network round-trips, 3 UI skeleton flashes after already having shown data.
...
Per `STASH_AUDIT.md` — all â³ PENDING:
...
| **PT-B** | TanStack Query for payment data (stash adoption) | ⚠ï¸ **Descoped** — see constraint C-1 | See C-1 |
...
| V-1 remaining | `isRefreshingPaymentMethods`, `paymentMethodsSnapshotReady`, `paymentMethodsRefreshKey` | â³ Deferred — C-1 |
