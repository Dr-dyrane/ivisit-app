...
| Payment/request handoff     | `useRequestFlow.js`, `useMapCommitFlow.js`, `emergencyRequestsService.js`, payment helpers                                                    | partial read   |
| Rating recovery             | `useTrackingRatingFlow.js`, `mapTracking.rating.js`, rating atoms, `MapModalOrchestrator.jsx`                                                 | mapped         |
| Contact dispatch            | chat atoms, chat hooks, `EmergencyContactDispatchModal.jsx`, `emergencyChatService.js`                                                        | partial read   |
| Backend/API contract        | emergency request RPCs, status guards, dispatch/ETA helpers, realtime projection helpers, runtime tests                                       | partial read   |
...
| Cancel path             | Current implementation evidence                                                                                                                            | Required proof                                                                                 |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
...
| Active ambulance cancel | `buildTrackingDestructiveAction()` can create an active ambulance cancel action when callbacks are available and the active surface is cancellable.        | Server row, active trip store, lifecycle, route atom, and header all clear for the same id.    |
| Active bed cancel       | `buildTrackingDestructiveAction()` can create an active bed cancel action when callbacks are available and the active surface is cancellable.              | Bed booking store and active map request clear without disturbing unrelated ambulance state.   |
...
| Contact Dispatch realtime lifecycle         | chat room subscription                  | visible + room + lifecycle gates              | expected close only on modal close       | open                   |
...
| Cancel pending approval                     | cancelled request/payment/visit         | pending approval cleared                      | pending sheet and header close           | needs review           |
| Cancel active request                       | cancelled status                        | store clears active trip                      | tracking chrome closes                   | needs review           |
| Cancel active bed                           | cancelled bed/visit state               | active bed store clears                       | bed tracking chrome closes               | needs review           |
...
| Visit detail resume id mismatch             | selected history request vs active row  | selected id matches active trip id            | no wrong-trip resume                     | open                   |
