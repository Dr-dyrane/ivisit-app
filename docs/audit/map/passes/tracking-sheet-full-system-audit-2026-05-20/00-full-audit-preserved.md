...
| Contact dispatch            | chat atoms, chat hooks, `EmergencyContactDispatchModal.jsx`, `MapModalOrchestrator.jsx`, `emergencyChatService.js`, emergency chat RPCs                                                       | mapped, live UI run open   |
| Backend/API contract        | emergency request RPCs, status guards, dispatch/ETA helpers, chat RPCs, realtime projection helpers, runtime tests                                                                            | mapped, runtime tests open |
...
| History/detail resume        | Selected history matching exists in one path, but detail resume still has a global active-trip guard. | Mapped; selection path is id-aware, detail resume remains a targeted fix candidate.                         |
| Rating close semantics       | Close clears rating atom without the skip/submit cleanup path.                                        | Product decision captured: close means skip, defer, or temporary dismiss, with matching cleanup.            |
...
| `handleRequestComplete()` starts runtime  | The request flow starts ambulance or bed runtime with canonical id, display id, status `accepted`, route/ETA, hospital coordinate, patient location, triage, responder assignment, and route fallback.    | Immediate tracking exists before the query returns, but it must be merge-compatible with server hydration.                |
...
| Close is not completion cleanup            | `closeTrackingRating()` only resets `trackingRatingStateAtom`; skip/submit are the paths that call `finalizeCompletedTracking()` and stop the active trip/bed.                                      | Pressing shell close can dismiss the modal without running skip/submit cleanup, so close semantics remain an open product decision.                    |
...
| Action failure feedback          | Prevent loading states from silently resetting after blocked arrival/cancel.  | `runBusyAction`, emergency handlers, tracking controller toast paths               | failed/blocked actions show visible feedback while preserving state        |
| Bed capacity vs ready state      | Prevent local hold-timer readiness from being mistaken for backend capacity.  | `useBedBookingProgress`, bed request handlers, hospital capacity triggers          | bed ready/check-in/capacity cards have distinct meanings                   |
...
These are the audit findings that match the user-visible problem class: the
app can be technically working while two UI surfaces tell different stories.

| Disharmony                                      | Backend truth may be valid? | API/store truth may be valid? | UI contradiction                                                                | Current code source                                                                                           | Fix shape                                                                                                         |
| ----------------------------------------------- | --------------------------- | ----------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Arrived says "confirm arrival" after confirmed  | yes                         | yes                           | hero subtitle can describe the previous action while CTA says complete request  | broad `TRACKING_STAGES.ARRIVED` copy without action context; fixed by `canMarkArrived`/`canCompleteAmbulance` | Keep broad stage, but render hero subtitle from action context.                                                   |
| Header says `En Route` while sheet is assigning | yes                         | yes                           | floating header overstates dispatch when no responder exists                    | `mapActiveSessionPresentation.js` collapses non-pending active ambulance to `En Route`                        | Feed header from tracking snapshot/stage, or replicate responder-aware stage refinement.                          |
| Tracking survives completion/rating cleanup     | yes                         | yes                           | live chrome can remain while rating/terminal flow owns the user moment          | key-gated `useMapTrackingHeader()` and close logic; query can preserve terminal trip for rating               | Derive `isTrackingSessionActive = trackingRequestKey && hasActiveTrip`; keep rating as separate terminal overlay. |
| Correct request id, wrong provider label        | yes                         | yes                           | sheet/header/details can show stale selected hospital for an active request     | active model resolves `preferredHospital`/payload before lookup by active request `hospitalId`                | When active request exists, lookup by `hospitalId` first; payload fallback only if id matches or lookup missing.  |
| Contact Dispatch appears to break ETA           | usually yes                 | usually yes                   | header hides, chat realtime closes, tracking remount can make ETA seem unstable | chat modal hides header via modal state and runs separate room realtime lifecycle                             | Treat chat as modal-only; prove request key/route atom/snapshot unchanged across open/close/send.                 |
| History resume opens wrong live request         | yes                         | yes                           | selected historical item can become a generic "open current tracking" action    | visit selection path is id-aware; detail resume handler only checks `hasActiveTrip`                           | Require selected visit id to match active request keys before calling `openTracking()`.                           |
| Rating close flashes/settles                    | yes                         | yes                           | closing or remounting rating can expose terminal tracking context for a frame   | `closeRating()` clears atom only; skip/submit perform final cleanup                                           | Decide whether close means defer or skip; if it means terminal dismiss, run the same cleanup semantics.           |
...
| Companion action after arrival              | arrived request or completion-ready bed       | action policy currently allows broad non-terminal add-ons           | no operationally stale companion CTA             | open targeted fix         |
...
| Visit detail resume id mismatch             | selected history request vs active row        | selection path matches partial key set; detail resume still global  | no wrong-trip resume                             | open targeted fix         |
