...
**Scope**: Wide â€” tracking â†’ visit detail handoff Â· rating modal chain Â· re-booking entry Â· visit stack pages  
...
**Guide**: `TRACKING_SHEET_LEARNINGS.md` defect classes 2.1â€“2.14 Â· `PRE_TRACKING_PHASE_AUDIT_2026-04-27_FINAL.md` constraint list.
...
Tracking sheet (useMapTrackingController)
  â†’ Complete CTA â†’ handleCompleteAmbulanceWithRating / handleCompleteBedWithRating
    â†’ deferCleanup â†’ writeTrackingRatingRecoveryClaim â†’ setRatingState (Jotai atom)
      â†’ ServiceRatingModal (lifted to MapScreen via useTrackingRatingFlow)
        â†’ submitRating / skipRating â†’ finalizeCompletedTracking (stopAmbulanceTrip / stopBedBooking)
          â†’ Zustand cleared â†’ tracking auto-open effect fires (isTripActive=false) â†’ EXPLORE_INTENT

History item tap (MapScreen handleSelectHistoryItem)
  â†’ canResumeLiveRequest? â†’ openTracking()
  â†’ else â†’ setSelectedHistoryVisitKey â†’ openVisitDetail(historyItem)
    â†’ MapVisitDetailStageBase â†’ useMapVisitDetailModel
      â†’ canResume â†’ onResume (handleResumeHistoryRequest â†’ openTracking)
      â†’ canRate â†’ onRateVisit (handleRateHistoryVisit â†’ historyRatingState modal)
      â†’ canBookAgain â†’ onBookAgain (handleBookAgainFromVisit)
        â†’ [BRIDGE] router.push('/(user)/(stacks)/book-visit')

Recovery path (MapScreen useEffect)
  â†’ pendingRecoveredRatingVisit â†’ setRecoveredRatingState â†’ recovered ServiceRatingModal
```

**Files audited**:
- `hooks/visits/useVisitHistorySelectors.js` â€” `canResume`, `canRate`, `canBookAgain` derivation
- `components/map/surfaces/visitDetail/useMapVisitDetailModel.js` (1402 lines)
- `components/map/views/tracking/useMapTrackingController.js` â€” rating flow
- `hooks/map/exploreFlow/useTrackingRatingFlow.js` â€” screen-level rating handler
- `hooks/map/exploreFlow/useMapTracking.js` â€” auto-open + closeTracking
- `hooks/map/exploreFlow/useMapSheetNavigation.js` â€” openVisitDetail / closeVisitDetail
- `screens/MapScreen.jsx` â€” orchestration, selectedHistoryVisitKey, recovery rating
- `components/map/views/visitDetail/MapVisitDetailStageBase.jsx` â€” shell
...
### VD-1 â€” `canResume` derived from visit status only â€” not from live Zustand trip state
**Severity**: ðŸ”´ HIGH  
**Defect class**: 2.2 (data race at state transition boundary)  
**Files**: `useVisitHistorySelectors.js` line 506, `useMapVisitDetailModel.js` line 1018, `MapScreen.jsx` lines 376â€“383
...
- Zustand: `activeAmbulanceTrip = null` â†’ `hasActiveTrip = false` â†’ tracking auto-open effect fires â†’ EXPLORE_INTENT
- Visit selector: `canResume = true` (status still `active` in local cache)
- Visit detail sheet: "Track" button enabled â†’ user taps â†’ `handleResumeHistoryRequest` â†’ `openTracking()` â†’ tracking sheet opens with no active trip â†’ blank/empty tracking surface

**MapScreen double-check (line 376â€“383)**: `handleSelectHistoryItem` has its own `canResumeLiveRequest` guard that checks `activeMapRequest?.hasActiveRequest`. This guard is correct â€” it bypasses visit detail and calls `openTracking()` directly. **But it only runs at tap time**. If the user already has the visit detail sheet open when the trip completes, `canResume` stays true until the visit selector re-derives. The "Track" button in the already-open detail stays live.
...
### VD-2 â€” `handleRateHistoryVisit` opens a separate `historyRatingState` modal â€” parallel rating path
**Severity**: ðŸ”´ HIGH  
**Defect class**: 2.14 (duplicate state machine for same operation)  
**Files**: `MapScreen.jsx` lines 1054â€“1080, `useTrackingRatingFlow.js`

There are **two independent rating paths** in the codebase:

1. **In-flow path** (from tracking sheet completion): `setRatingState` (Jotai `trackingRatingStateAtom`) â†’ `useTrackingRatingFlow` â†’ lifted `ServiceRatingModal` at screen root. Has `deferCleanup`, `completionCommitted`, recovery claim write/delete.

2. **History visit detail path** (from visit detail "Rate" button): `handleRateHistoryVisit` â†’ `setHistoryRatingState` (`useState` in `MapScreen`) â†’ separate `ServiceRatingModal` instance. Does NOT call `updateVisit`. Does NOT write/delete recovery claim. Does NOT call `stopAmbulanceTrip`/`stopBedBooking`. Does NOT check `completionCommitted`.
...
**If a user taps Rate from the visit detail sheet**: rating modal opens, user submits â€” but `updateVisit` to `COMPLETED` may not fire (depends on which `ServiceRatingModal` submit handler is wired). The visit stays `rating_pending` in Supabase. Next time the visit detail opens, `canRate` is still true. Duplicate ratings possible.

**Gold Standard fix**: Consolidate both rating paths into `useTrackingRatingFlow`. History visit detail "Rate" button should call `openRatingForVisit(historyItem)` â€” a new entrypoint on the single rating flow that sets `completionCommitted: true` (completion already happened), skips `deferCleanup`, and runs the same `resolveTrackingRatingSubmit` path. **Pass: VD-C**.

---

### VD-3 â€” `selectedHistoryVisitKey` is `useState` â€” survives across sheet phases, bleeds into wrong surface
**Severity**: ðŸŸ¡ MEDIUM  
**Defect class**: 2.8 (state surviving wrong mount cycle)  
**Files**: `MapScreen.jsx` lines 170, 253â€“256, 386â€“388, 400â€“404
...
**Gold Standard fix**: `selectedHistoryVisitKey` belongs in a Jotai atom reset on `closeVisitDetail`. Alternatively â€” reset it whenever `sheetPhase !== VISIT_DETAIL`. **Pass: VD-B**.

---

### VD-4 â€” `useMapVisitDetailModel` has two `useState` + async `useEffect` API calls â€” layer violations
**Severity**: ðŸŸ¡ MEDIUM  
**Defect class**: V-1 (API-derived state in useState + useEffect)  
**Files**: `useMapVisitDetailModel.js` lines 514â€“536, 823â€“860

**Violation 1 â€” Hospital detail fetch** (lines 514â€“536):
```js
...
**Violation 2 â€” Payment history fallback fetch** (lines 823â€“860):
```js
...
```
Belongs in TanStack Query. The `localPaymentTotalLabel` dep means this fires again whenever the local label changes â€” including when the same visit is re-rendered after unrelated state changes.

**Gold Standard fix**: Both â†’ `useQuery` with stable keys. `useHospitalDetailQuery(hospitalId)` and `usePaymentHistoryEntryQuery(paymentLookupKey)`. **Pass: VD-D**.

---

### VD-5 â€” `canBookAgain` bridges to legacy full-screen route â€” sheet-native re-booking not implemented
**Severity**: ðŸŸ¡ MEDIUM  
**Defect class**: architectural gap  
**Files**: `MapScreen.jsx` line 344â€“350 (comment + handler), `useVisitHistorySelectors.js` line 512

```js
// MapScreen â€” "temporarily bridged"
...
`MapSheetOrchestrator` â†’ `MapVisitDetailStageBase` props â†’ `onBookAgain` â€” need to verify this is actually wired or silently null. If null, the "Book again" button in the detail sheet is dead but shows as enabled because `canRevisit = Boolean(historyItem?.canBookAgain && typeof onBookAgain === "function")` â€” if `onBookAgain` is undefined, `canRevisit = false` and button is disabled. But if a stub `() => {}` is passed, the button appears active and does nothing.

**Gold Standard fix**: Wire `onBookAgain` from visit detail to `openBookingFromVisit(historyItem)` â€” a new handler that pre-fills the commit wizard with the previous visit's hospital and service type. **Pass: VD-E (future â€” depends on sheet-native booking pass)**.

---

### VD-6 â€” `handleResumeHistoryRequest` calls `openTracking()` without verifying `hasActiveTrip`
**Severity**: ðŸ”´ HIGH  
**Defect class**: 2.2 (data race)  
**Files**: `MapScreen.jsx` lines 1049â€“1052
...
### VD-7 â€” Recovery rating mechanism uses `useState` + `useMemo` chain with non-deterministic trigger
**Severity**: ðŸŸ¡ MEDIUM  
**Defect class**: 2.2 (non-deterministic state trigger)  
**Files**: `MapScreen.jsx` lines 166â€“169, 926â€“957
...
**Problem**: `ratingRecoveryClaims` is `useState` seeded from AsyncStorage â€” loaded asynchronously on mount. If the `visits` selector resolves before `ratingRecoveryClaims` loads, `pendingRecoveredRatingVisit` returns null (no claims to match against) â†’ no recovery modal. When claims load, `useMemo` re-runs but may not retrigger the `useEffect` if `pendingRecoveredRatingVisit` identity hasn't changed. The recovery window is non-deterministic.

**Also**: `handledRecoveredRatingVersion` is a `useState` counter used as a useMemo dep to force re-evaluation. This is an anti-pattern â€” a version counter as a reactive dep is a `useReducer` / Jotai atom in disguise.
...
### VD-8 â€” `useMapVisitDetailModel` is 1402 lines â€” architectural violation
**Severity**: ðŸŸ¡ MEDIUM  
**Defect class**: file architecture violation (>1000 lines = mandatory refactor candidate)  
**Files**: `useMapVisitDetailModel.js`

Per gold standard rules: >1000 lines = architectural violation (unless generated). This file owns: theme tokens, 12+ pure helper functions (200 lines), two async data fetches (violations VD-4), presentation builders for hero/journey/triage/payment/actions/stats â€” all in one hook.

**Gold Standard fix**: Split into:
- `visitDetail.helpers.js` â€” pure formatters (`formatHumanWhen`, `resolveJourneyProgress`, etc.)
- `visitDetail.builders.js` â€” `buildJourney`, `buildTriageRows`, `buildVisitCollapsedAction`, etc.
- `useHospitalDetailQuery` / `usePaymentHistoryEntryQuery` â€” extracted queries (VD-4)
- `useMapVisitDetailModel.js` â€” slim orchestration only (~200 lines)

**Pass: VD-D**.

---

### VD-9 â€” `finalizeCompletedTracking` is duplicated in controller and screen-level flow
**Severity**: ðŸŸ¡ LOW  
**Defect class**: DRY violation  
**Files**: `useMapTrackingController.js` lines 411â€“422, `useTrackingRatingFlow.js` lines 56â€“67
...
### VD-10 â€” `skipRating` / `submitRating` duplicated between controller and screen-level flow
**Severity**: ðŸŸ¡ MEDIUM  
**Defect class**: DRY violation + dual ownership  
**Files**: `useMapTrackingController.js` lines 424â€“535, `useTrackingRatingFlow.js` lines 73â€“156
...
**Gold Standard fix**: Single `useTrackingRatingFlow` at screen root owns all rating logic. Remove `skipRating`/`submitRating`/`closeRating`/`finalizeCompletedTracking` from `useMapTrackingController` â€” controller only sets `ratingState` atom (open). Screen-level flow handles all close/skip/submit paths. **Pass: VD-C**.

---

### EC-VD-1 â€” Tracking â†’ visit detail handoff: no `historyItem` at sheet open time for live trips
**Severity**: ðŸ”´ HIGH  
**Files**: `MapScreen.jsx` lines 352â€“389, `useMapSheetNavigation.js` lines 237â€“248
...
`handleSelectHistoryItem` calls `openVisitDetail(historyItem)` even if `historyItem` is sparse. `useMapVisitDetailModel` receives a sparse object â†’ hero renders blank facility name â†’ payment row shows null â†’ journey is null. No error thrown.
...
### EC-VD-2 â€” `closeVisitDetail` goes directly to EXPLORE_INTENT â€” back navigation breaks
**Severity**: ðŸŸ¡ MEDIUM  
**Files**: `useMapSheetNavigation.js` lines 246â€“248
...
### UX-VD-1 â€” "Track" button enabled for completed trip (maps to VD-1, VD-6)
...
### UX-VD-2 â€” "Rate" opens different modal depending on entry path (maps to VD-2)
...
### UX-VD-3 â€” Closing visit detail loses history list context (maps to EC-VD-2)
...
### UX-VD-4 â€” Visit detail shows blank/sparse hero for live trips during Realtime sync gap (maps to EC-VD-1)
**HIG standard**: A surface in a loading state should communicate loading, not render empty/broken content.  
**Fix path**: VD-B â€” skeleton guard + minimum field check.

### UX-VD-5 â€” Recovery rating modal can interrupt new booking flow (prior audit finding)
**HIG standard**: A modal from a prior session must not interrupt an active user intent.  
**Fix path**: VD-C â€” `canRecoverTrackingRating` guard already partially handles this; needs `sheetPhase === COMMIT_PAYMENT` added to the block list.
...
| `handledRecoveredRatingVersion` | `useState` counter | Remove â€” Jotai atom | VD-7 |
...
| **VD-G** | HIG polish + DRY | VD-9, UX-VD-1â€“5 | `mapTracking.rating.js`, UI components | Low |

---

## 5. Verified Clean

- **`useMapTrackingController` deferCleanup pattern** âœ… â€” `onCompleteAmbulanceTrip({ deferCleanup: true })` correctly defers `stopAmbulanceTrip` until after rating modal closes. `finalizeCompletedTracking` only runs post-submit/skip. Rating state context (hospitalTitle, providerName) is captured from Zustand BEFORE `stopAmbulanceTrip` clears it.
- **`trackingRatingStateAtom` persistence** âœ… â€” Jotai atom survives sheet phase transitions. `ServiceRatingModal` lifted to screen root by Pass B. Rating modal cannot be interrupted by sheet phase change.
- **`writeTrackingRatingRecoveryClaim` before `setRatingState`** âœ… â€” claim is persisted to AsyncStorage BEFORE the atom is set, so a crash between the two still has a recoverable claim.
- **`handleSelectHistoryItem` live trip guard** âœ… â€” `canResumeLiveRequest` guard correctly routes live trips to `openTracking()` instead of `openVisitDetail()` at tap time.

---

## 6. New Defect Classes (Append to TRACKING_SHEET_LEARNINGS.md)

- **2.15** â€” Duplicate rating paths. Two independent rating state machines (`trackingRatingStateAtom` path + `historyRatingState` useState path) for the same operation with divergent post-completion side effects. Rule: one operation = one state machine = one entrypoint. Extra entrypoints must funnel into the canonical path.
- **2.16** â€” `canResume` derived from visit DB status only, not live Zustand trip state. After trip cleanup, visit DB status lags Zustand by one Realtime cycle. Any CTA derived from visit status alone is incorrect for live-trip transitions. Rule: live-trip action capability must be gated on Zustand identity, not visit row status.
...
STATUS: AUDIT COMPLETE â€” READY FOR VD-A

DEFECT SUMMARY:
  HIGH:    VD-1 (canResume stale â€” Track button enabled post-trip)
           VD-2 (dual rating paths â€” divergent side effects)
  ...
  MEDIUM:  VD-3 (selectedHistoryVisitKey wrong layer)
           ...
           VD-5 (canBookAgain bridge â€” sheet-native not wired)
           VD-7 (recovery rating mechanism non-deterministic)
           VD-8 (model file 1402 lines â€” architectural violation)
           VD-10 (skipRating/submitRating duplicated â€” risk: completionCommitted gap)
...
NEW DEFECT CLASSES:
  2.15 â€” Duplicate rating paths
  2.16 â€” canResume from visit status only (not Zustand)

NEXT:
  VD-A â€” Add diagnostic logging only:
