...
- `npx supabase db lint --linked --schema public --fail-on error` is currently blocked by pre-existing remote lint errors in PostGIS/legacy functions; the edited Contact Dispatch SQL is not live on remote yet, so this lint run does not validate the new RPCs.
