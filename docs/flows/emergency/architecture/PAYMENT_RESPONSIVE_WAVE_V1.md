...
> Status: Active â€” wave in progress
...
Payment is already modular (composition root â†’ hook â†’ orchestrator â†’ stage base â†’ variants â†’ tokens). Adding the responsive layer on top gives three wins at once:
...
- Management mode content (wallet balance, link card, saved methods, history) is **identical across variants** â€” only card widths, column counts, and gutters change
- Checkout mode content (identity, summary, methods, footer CTA) is **identical across variants** â€” only max-width and vertical rhythm change
...
- Full Shape A would produce 2 phases Ã— 14 variants = **28 view files**. Unacceptable.
...
- Presentation is shallow â€” the orchestrator selects a phase variant and renders.
- **Answer: shallow.**
...
- No new view files â€” existing variants read responsive metrics from config
...
- `STACK_VIEWPORT_VARIANTS` â€” same 14 names as Map/Welcome
- `getStackViewportVariant({ platform, width })` â€” resolver mirroring `getMapViewportVariant`
- `getStackViewportSurfaceConfig(variant)` â€” returns numeric / enum primitives per variant
...
- `contentMaxWidth` â€” max width for the scroll column
- `contentHorizontalPadding`
- `cardGap` â€” vertical gap between stacked cards
- `columnCount` â€” 1 for compact, 2 for tablet, 2 or 3 for desktop
- `modalPresentationMode` â€” `bottom-sheet` | `centered-modal` | `side-drawer`
...
- Typography â€” title / heading / body / caption sizes + line-heights
- Spacing â€” `stackGap`, `cardPadding`, `sectionGap`
- Card sizing â€” `iconSize`, `orbSize`, `buttonHeight`, `inputHeight`
- Radii â€” squircle radius scale per density
...
4. Wire `PaymentStageBase` to consume the config â€” enforce `contentMaxWidth` and horizontal padding
5. Wire `PaymentManagementVariant` to consume metrics â€” single column on compact, 2-column on tablet+, gutters from config
6. Wire `PaymentCheckoutVariant` to consume metrics â€” centered column with `contentMaxWidth`
...
- Refactoring any other screen â€” infrastructure is app-wide but only Payment is wired this wave
