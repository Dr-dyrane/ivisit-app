...
|-------|-------|----------|
| Issue 1 | Triage Questions Pre-Selected | 🔴 Open |
| Issue 2 | Room Card Layout in HALF Snap | 🔴 Open |
| Issue 3 | Saved Transport Card Bleeds Into Bed Decision | 🔴 Open |
| Issue 6 | Transport Screen Weak Hierarchy | 🟡 Open |
| Issue 7 | Transport Details Truncated Across Cards | 🟡 Open |
| §1.5 | MapTopLeftControl Back-Nav (architectural) | 🔴 Open |
...
1. `docs/REFACTORING_GUARDRAILS.md` — `useEffect` decision tree, subsequent pass rule
2. `docs/architecture/refactoring/TRACKING_SHEET_LEARNINGS.md` — defect class 2.14 (terminal state not locked)
3. `docs/architecture/refactoring/REFACTORING_BIBLE.md` — Commandment 2 (no anonymous function props), Commandment 8 (baseline hash before monolith edit)
4. `docs/flows/emergency/MAP_SCREEN_IMPLEMENTATION_RULES_V1.md` — `MapScreen.jsx` stays thin
...
| `components/map/views/commitTriage/useMapCommitTriageController.js` | Add `isFreshSession` guard — empty draft on fresh open, restore only on resume |
| `components/map/views/commitTriage/mapCommitTriage.helpers.js` | Add `buildFreshTriageDraft()` helper — returns empty dict keyed to step IDs |
| `components/map/views/bedDecision/MapBedDecisionStageBase.jsx` | HALF snap: RouteCard → EXPANDED-only; SavedTransportCard → status strip |
...
| `components/map/views/ambulanceDecision/mapAmbulanceDecision.content.js` | Audit title copy — task language `"Select Transportation"` |
| `components/map/views/bedDecision/mapBedDecision.content.js` | Audit sheet title copy — `"Choose Your Room"` |
...
Two init modes:
- **Resume:** `payload?.triageDraft` exists AND `payload?.requestId` matches an active trip → restore draft and step position
- **Fresh:** no `triageDraft` or no matching `requestId` → always `null`, always step 0

Add `buildFreshTriageDraft()` helper in `mapCommitTriage.helpers.js` — returns `{}` keyed to step IDs — for the reset path in the existing session-key effect.

**Invariants:**
- Step progression, live save, copilot prompt logic — entirely untouched
- CTA in `MapCommitTriageStageParts.jsx` must stay disabled until user actively selects an option for the current step — audit existing guard, do not remove it
...
**Root cause:** HALF snap renders four information clusters simultaneously: Hero, SavedTransportCard (full glass panel), RoomSwitchRow, and RouteCard. The RouteCard at line 429 is in the HALF else branch — always present — adding a second substantial card before the footer.

**HALF snap target:**
- Hero card
- Transport status strip (compact, not full card) — see Issue 3
- RoomSwitchRow
- "Show more" expand affordance (chevron + muted copy)

**EXPANDED snap target (unchanged):**
- Hero, SwitchRow (or `MapBedDecisionExpandedRoomChoices`), RouteCard, DetailsCard — full depth
...
Add "expand for more" affordance in HALF: a single `<TouchableOpacity>` below SwitchRow with `chevron-down` icon and `"More details"` caption in `secondaryText` style. Tap → `onExpandSheet()`. Not a new component — inline JSX, <30 lines.

---

## Issue 3 — Saved Transport Card Bleeds Into Bed Decision
...
**Fix — replace full card with `MapBedDecisionTransportStatusStrip`:**
```jsx
// PULLBACK NOTE: UX-A — SavedTransportCard demoted to status strip in HALF snap
// OLD: <MapBedDecisionSavedTransportCard /> — full glass panel in HALF
// NEW: <MapBedDecisionTransportStatusStrip /> — compact inline row in HALF
...
// - Copy: "Ambulance confirmed — [tier name]" in `caption` style
// - No glass panel container — inline row only
// - Tappable in HALF: onPress → navigates back to ambulance decision (secondary affordance)
...
## Issues 6 + 7 — Transport Hierarchy and Truncation

**Live files:** `MapAmbulanceDecisionStageBase.jsx`, `MapAmbulanceDecisionStageParts.jsx` (lines 266–335)

**Root cause:** Switch pills use `numberOfLines={1}` — truncates tier names. Hero description uses `numberOfLines={2}` — truncates service detail. Sheet title not audited against task language.

**Fixes (combined — same files):**
```js
// PULLBACK NOTE: UX-A — switch pill + hero description line cap adjustment
...
//      + "See full details →" link when description is actually truncated (onTextLayout)
```

Sheet title: audit `mapAmbulanceDecision.content.js` — confirm or update to `"Select Transportation"`.
Bed decision sheet title: audit `mapBedDecision.content.js` — confirm or update to `"Choose Your Room"`.

No accordion expansion of non-selected pills — existing pattern is correct and must not change.

---

## §1.5 — MapTopLeftControl Back-Nav

**Live files:** `screens/MapScreen.jsx` (line 597–605), `components/map/views/shared/MapTopLeftControl.jsx`
...
`handleDecisionBack`: extracted as `useCallback` in `MapScreen.jsx` — calls `buildExploreIntentSheetView()` or `closeDecisionPhase()` per the existing pattern in `useMapCommitFlow.js`. No anonymous function in prop.

**Safety invariant — WAITING_APPROVAL cannot be bypassed:** `COMMIT_PAYMENT` is explicitly excluded from `isDecisionPhase`. The PT-C fix is not touched. The chrome button is hidden in all commit phases.
...
| Track | Scope |
|-------|-------|
| State management | `useMapCommitTriageController.js` — initialization path only. `isDecisionPhase` derived inline — not `useEffect` (REFACTORING_GUARDRAILS: derived value → inline const). |
...
| Documentation | PULLBACK NOTE on every structural change. Pass log in `UX_ISSUES_SUBPASS_PLAN_2026-05-10.md §3` updated post-commit. |
...
| Rule | How complied |
|------|-------------|
| `MapScreen.jsx` stays thin | `isDecisionPhase` is two derived lines — not business logic |
...
- `MapBedDecisionSavedTransportCard` full-card preserved and rendered in EXPANDED — not deleted
...
- [ ] `MapTopLeftControl` — authenticated in `AMBULANCE_DECISION`: back chevron visible, returns to EXPLORE_INTENT
- [ ] `MapTopLeftControl` — authenticated in `COMMIT_PAYMENT`: control hidden entirely
- [ ] `MapTopLeftControl` — authenticated in `EXPLORE_INTENT`: avatar visible, opens profile
- [ ] `MapTopLeftControl` — unauthenticated in `EXPLORE_INTENT`: back chevron → `/(auth)/` (regression guard)
...
- [ ] No new `useEffect` added — any added must be justified against decision tree
- [ ] `MapBedDecisionStageParts.jsx` line count checked — if >950, extract transport strip
