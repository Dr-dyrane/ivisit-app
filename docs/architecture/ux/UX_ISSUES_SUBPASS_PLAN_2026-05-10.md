...
# UX Issues Sub-Pass Plan â€” 2026-05-10

**Status:** PLAN READY â€” awaiting execution approval  
**Scope:** Issues 1â€“9 + Issues 10â€“12 from `IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS_2026-05-10.md`  
...
1. `docs/REFACTORING_GUARDRAILS.md` â€” state layer rules, `useEffect` decision tree, loading state rule, subsequent pass rule
2. `docs/architecture/refactoring/TRACKING_SHEET_LEARNINGS.md` â€” defect classes 2.1â€“2.14, especially: 2.1 (modal gated on transient parent), 2.2 (imperative auto-open race), 2.13 (cascading useEffect churn), 2.14 (terminal state not locked)
3. `docs/architecture/refactoring/REFACTORING_BIBLE.md` â€” The Commandments (no >300-line files, no business logic in JSX, every pass has a git checkpoint)
4. `docs/flows/emergency/MAP_SCREEN_IMPLEMENTATION_RULES_V1.md` â€” one persistent map, one persistent sheet, one changing mode; `MapScreen.jsx` stays thin
5. `docs/architecture/ux/IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS_2026-05-10.md` â€” the source issue register this plan implements
...
## 0. Ground Truth â€” What Is Already Done

Before any pass begins, confirm these items are shipped and do not re-touch them:

| Item | File | Status |
|---|---|---|
| PT-6: `WAITING_APPROVAL` removed from `isCommitPaymentDismissibleState` | `mapCommitPayment.transaction.js` line 60â€“66 | âœ… DONE (PT-C) |
| PT-6: `awaitingApprovalRef` prevents `finally` reset during approval window | `useMapCommitPaymentController.js` | âœ… DONE (PT-C) |
| PT-2: `finishCommitPayment` calls `openTracking()` unconditionally | `useMapCommitFlow.js` | âœ… DONE (PT-E) |
| PT-D: `paymentAtoms` wired â€” `submissionState`, `estimatedCost`, `isLoadingCost` | `useMapCommitPaymentController.js` | âœ… DONE (PT-D) |
| PT-B2: Scalar deps fix for `loadCost` / `refreshPaymentMethodSnapshot` | `useMapCommitPaymentController.js` | âœ… DONE (PT-B2) |
| PT-12: Double `updateVisit` collapsed to single `MONITORING` write | `useRequestFlow.js` | âœ… DONE (PT-G) |
| ETA null guard | `useRequestFlow.js` | âœ… DONE (PT-G) |
| Explore Intent haptics (E-2.1â€“2.5) | `MapExploreIntentCareSection.jsx`, `MapExploreIntentHospitalSummaryCard.jsx`, `MapExploreIntentStageParts.jsx`, `MapExploreIntentProfileTrigger.jsx` | âœ… DONE (HIG Pass A) |
| Explore Intent `reduceMotion` gate + spring selection (E-2.6, E-2.8) | `MapExploreIntentStageBase.jsx`, `MapExploreIntentCareSection.jsx` | âœ… DONE (HIG Pass B) |
| Explore Intent skeleton timeout fallback + orb dimming (E-2.9, E-2.10) | `MapExploreIntentHospitalSummaryCard.jsx`, `MapExploreIntentCareSection.jsx` | âœ… DONE (HIG Pass C) |
| Explore Intent accessibility labels + live region (E-2.11â€“2.13) | `MapExploreIntentCareSection.jsx`, `MapExploreIntentHospitalSummaryCard.jsx` | âœ… DONE (HIG Pass D) |
| Service selection footer dock horizontal padding removed | `mapBedDecision.styles.js`, `mapAmbulanceDecision.styles.js`, `useMapStageResponsiveMetrics.js`, `MapBedDecisionStageParts.jsx`, `MapAmbulanceDecisionStageParts.jsx` | âœ… DONE (Quick fix â€” CTA row full-width without overflow) |
...
| C-2 / CV-2 | Remove `isSubmitting` boolean â€” derive all from `submissionState.kind` | UX-D |
| C-3 / PT-4 | Atomic `transitionPendingToActive` Zustand action | UX-D |
| C-4 / PT-3 | `commitFlow` â†’ Jotai atom (session-ephemeral) | UX-D |
| C-5 / EC-2 | Ghost settlement path â€” `FINALIZING_DISPATCH` UI surface | UX-C |
| C-6 / PT-7 | Stable `Math.random()` display ID â€” `useRef` per mount | UX-C |
...
## 1. Issue Register â€” Current State

### Issue 1 â€” Triage Questions Pre-Selected

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 1  
**Live file:** `components/map/views/commitTriage/useMapCommitTriageController.js`  
**Status:** ðŸ”´ Open â€” not addressed in any prior pass

**Root cause (confirmed in code):**

`initialDraft` (lines 73â€“102) reads from 11 chained fallback sources:
```
payload?.triageDraft
...
**Fix scope (UX-A):**
- Add `isFreshSession` prop to the controller. When `true`, `initialDraft` is forced to `null` regardless of the fallback chain â€” options start neutral.
- Distinguish two init modes:
  - **Resume** (`payload?.triageDraft` exists and `payload?.requestId` matches an active trip) â€” restore draft and step position. This is intentional recovery.
  - **Fresh** (no `payload?.triageDraft` or no matching `requestId`) â€” always start with empty draft, always open at step 0.
- The existing session-key effect (lines 121â€“130) correctly resets when the session changes â€” this already handles the re-open scenario if the session key is stable.
...
**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 2  
...
1. `MapBedDecisionHero` â€” hero room card (always)
2. `MapBedDecisionSavedTransportCard` â€” when `careIntent === "both"` (always, at this snap state)
3. `MapBedDecisionRoomSwitchRow` â€” when `decision.roomOptions.length > 1` (always in HALF)
4. `MapBedDecisionRouteCard` â€” in the HALF else branch (line 429) â€” always present

Four information clusters visible simultaneously in a half-height sheet. The `MapBedDecisionDetailsCard` is correctly EXPANDED-only (good), but the RouteCard at line 429 renders in HALF as well â€” adding a second substantial card below the switch rail before the footer.
...
- EXPANDED: Hero, SwitchRow (or `MapBedDecisionExpandedRoomChoices`), RouteCard, DetailsCard â€” full depth.

---

### Issue 3 â€” Saved Transport Card Bleeds Into Bed Decision (Combined Flow)

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 3  
**Live files:** `MapBedDecisionStageBase.jsx` (lines 357â€“371), `MapBedDecisionStageParts.jsx` (lines 551â€“629)  
**Status:** ðŸ”´ Open

**Root cause (confirmed in code):**

`MapBedDecisionSavedTransportCard` renders a full glass panel card with icon, title, meta row, and service name when `careIntent === "both"`. It occupies approximately the same visual weight as the Hero card â€” two full-weight cards in the bed decision context.
...
- Replace `MapBedDecisionSavedTransportCard` (full card) with a compact inline status strip:
  ...
  - Copy: `"Ambulance confirmed â€” [tier name]"` or `"Transport: [title]"` in muted caption style
  - No glass panel, no card container â€” inline row beneath Hero, above SwitchRow
...
- The step header in the bed decision sheet title area should say `"Choose your room"` â€” not surface transport context

---

### Issue 4 â€” Hospital Badges Lack Meaning and Hierarchy

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 4  
**Live file:** `components/map/surfaces/hospitals/mapHospitalDetail.helpers.js` (lines 141â€“155)  
**Status:** ðŸŸ¡ Open â€” lower priority, no patient safety impact
...
**Fix scope (UX-B):**
- Reorder badge priority: decision-critical signals first (emergencyLevel, wait time, ETA, distance) â†’ trust/quality signals last (Verified, Standard).
- Pair each capability badge with a tooltip/accessible hint: `"Level 2 emergency center â€” handles complex trauma"`.
- `"Standard"` badge: only show when the hospital has no higher capability tier. When it renders, give it muted/secondary styling â€” it signals absence of specialty, not a positive feature.

---

### Issue 5 â€” Mixed Entity Data in Hospital Cards

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 5  
**Live file:** `components/map/views/exploreIntent/MapExploreIntentHospitalSummaryCard.jsx`, `MapHospitalListContent.jsx`  
**Status:** ðŸŸ¡ Open â€” lower priority
...
**Fix scope (UX-B):**
- Hospital cards (list rows and summary card): hospital-specific data only â€” name, distance, ETA, rating, price tier, emergency level badge.
- Network totals (`nearbyHospitalCount`, `totalAvailableBeds`, `nearbyBedHospitals`) belong only in the explore intent orb subtexts and the care intent summary section â€” not inside individual hospital cards.
- Audit `MapExploreIntentHospitalSummaryCard.jsx` and `MapHospitalListContent.jsx` for any cross-boundary data renders.

---

### Issue 6 â€” Transport Screen Weak Hierarchy

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 6  
**Live files:** `MapAmbulanceDecisionStageBase.jsx`, `MapAmbulanceDecisionStageParts.jsx`  
**Status:** ðŸŸ¡ Open

**Root cause (confirmed in code):**

The ambulance decision sheet title comes from `mapAmbulanceDecision.content.js` â€” not dynamically set to `"Select Transportation Option"`. Switch pills in `MapAmbulanceDecisionSwitchRow` use `numberOfLines={1}` which truncates tier names. The `MapAmbulanceDecisionHero` description uses `numberOfLines={2}` â€” truncates service detail at 2 lines.

**Fix scope (UX-A):**
- Sheet title: ensure the `MapAmbulanceDecisionTopSlot` title always reads the task-language label `"Select Transportation"` (or equivalent from content file â€” audit current copy).
...
- Hero description: 3â€“4 line cap instead of 2. Add `"See full details â†’"` link to `ServiceDetail` only when description is actually truncated.
...
### Issue 7 â€” Transport Details Truncated Across Cards

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 7  
**Live files:** `MapAmbulanceDecisionStageParts.jsx` (lines 266â€“335: `MapAmbulanceDecisionSwitchRow`)  
**Status:** ðŸŸ¡ Partially addressed by existing SwitchRow+Hero accordion
...
**Fix scope (UX-A):** Combined with Issue 6 â€” same pass. Accordion discipline is already implemented. Polish only:
- Pills: `numberOfLines={1}` â†’ allow wrapping on 2 lines for pill labels, or use visual shortLabel from content
- Hero: 3-line description cap with "more" affordance
- No accordion expansion of non-selected pills â€” existing pattern is correct

---

### Issue 8 â€” Payment Progression CTA

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 8  
**Live files:** `MapCommitPaymentStageParts.jsx` (lines 216â€“275), `MapCommitPaymentStageBase.jsx`  
**Status:** ðŸŸ¡ Partially addressed (CTA exists, label and sticky positioning need audit)

**Root cause (confirmed in code):**

`MapCommitPaymentSummaryCard` receives `primaryActionTitle` and renders `EntryActionButton` at the bottom of the card â€” inside the scroll body, not in a sticky footer. When the payment sheet is in HALF snap (default on non-sidebar layouts), the CTA can fall below the visible scroll viewport if the card content is long.

The CTA label flows from `mapCommitPayment.content.js` â†’ `MAP_COMMIT_PAYMENT_COPY` â†’ needs audit. Deferred items C-5 (`FINALIZING_DISPATCH` surface), C-6 UX-5 (wallet disabled caption), C-6 UX-6 (Dynamic Type truncation) are all in scope here.

**Fix scope (UX-C):**
- Move terminal payment CTA to a sticky footer outside the scroll body â€” matches the LocationSheet guardrail rule.
- CTA label: task-language â€” `"Confirm & Dispatch"` (cash/approval), `"Pay $X.XX"` (card) â€” not generic `"Continue"`. Audit `MAP_COMMIT_PAYMENT_COPY` constants.
- Add `FINALIZING_DISPATCH` UI surface for EC-2 ghost settlement path (carry-forward C-5).
- UX-5: Wallet method shown as disabled row with `"Balance $X.XX â€” not enough for $Y.YY"` caption. Never silently hidden.
- UX-6: CTA label layout â€” cost on one line, action verb on second line at large Dynamic Type. Audit with Accessibility Inspector.

---

### Issue 9 â€” Navigation Stack Resets State

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 9  
**Live files:** `stores/emergencyTripStore.js`, `useMapCommitFlow.js`, `mapCommitPayment.transaction.js`  
**Status:** ðŸŸ¡ Mostly addressed by Zustand + commitFlow restore. One live gap remains.

**Root cause:**

PT-3 / C-4 (deferred): `commitFlow` persists in Zustand (layer 3), not Jotai (session-ephemeral). This means back navigation within the map sheet restores correctly during the session (Zustand survives navigation), but cold starts see a stale restore (EC-1). The most dangerous state was PT-6 (WAITING_APPROVAL dismissible) â€” already fixed.
...
- This is the correct fix for Issue 9 â€” it makes back-navigation state restoration session-scoped only, never cross-session.

---

### Issue 10 â€” OTP CTA Timing In Emergency Commit

**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 10  
...
- CTA state machine: `idle` â†’ `pressed` (haptic) â†’ `requesting` (spinner, OTP call fires) â†’ `ready` (OTP sheet opens) â†’ `failed/retry`.
...
**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 11  
**Live files:** Mini profile surface (TBD â€” not yet identified in audit)  
**Status:** ðŸ”µ Deferred â€” LocationSheet must be stable first

**Fix scope (UX-E â€” LocationSheet dependent):**
...
**Source doc:** IVISIT_UX_ISSUE_MAPPING_AND_LOCATION_GUARDRAILS Â§Issue 12  
**Live files:** `MapPhaseTransitionView`, all `*StageBase.jsx` files, `WelcomeScreen`, `WelcomeMapHandoffCover`, root/auth stack layouts  
**Status:** ðŸŸ¡ Open â€” systemic pattern issue; Welcome -> Map route handoff patched
...
- Sheet phase transition contract required for all phases:
  - `COMMIT_TRIAGE` â†’ `COMMIT_PAYMENT`: preserve shell, fade body
  - `BED_DECISION` â†’ `COMMIT_DETAILS`: preserve shell, slide body
  - `AMBULANCE_DECISION` â†’ `COMMIT_TRIAGE`: preserve shell
- Loading states: skeleton rows not blank white â€” audit all `isLoading` + blank body patterns.
- Web sheet swap: verify `MapPhaseTransitionView` is in the render tree for all phase changes on web.

**Resolved subcase â€” Welcome -> Map empty frame:**
...
## 1.5 MapTopLeftControl â€” Back-Navigation Capability (Architectural Note)

**File:** `components/map/views/shared/MapTopLeftControl.jsx`  
**Rendered in:** `screens/MapScreen.jsx` line 597â€“605  
...
- Unauthenticated: renders `chevron-back` â†’ `router.replace("/(auth)/")`
- Authenticated: renders user avatar â†’ `onOpenProfile`
- Hidden entirely once the sheet enters any focused phase (bed/ambulance decision, commit, tracking).

**What the user flagged:** This control already exists as a floating map-chrome button on the left side. It should be extended to provide back-navigation for **authenticated** users across multiple sheet phases â€” not just for unauthenticated users going back to Welcome.

**Architectural opportunity â€” what to expand in UX-A:**
...
|---|---|---|
| `EXPLORE_INTENT` | chevron-back â†’ Welcome | avatar â†’ profile |
| `AMBULANCE_DECISION` | â€” (not reachable) | chevron-back â†’ EXPLORE_INTENT |
| `BED_DECISION` | â€” (not reachable) | chevron-back â†’ EXPLORE_INTENT |
| `HOSPITAL_LIST` | â€” (not reachable) | chevron-back â†’ EXPLORE_INTENT |
| `HOSPITAL_DETAIL` | â€” (not reachable) | chevron-back â†’ previous phase |
| `COMMIT_DETAILS` | â€” (not reachable) | **Not shown** â€” sheet header owns the back button in deep commit phases |
| `COMMIT_TRIAGE` | â€” (not reachable) | **Not shown** â€” sheet header owns the back button |
| `COMMIT_PAYMENT` | â€” (not reachable) | **Not shown** â€” WAITING_APPROVAL lock must not be bypassed via chrome |
| `TRACKING` | â€” (not reachable) | **Not shown** â€” tracking is a terminal state, no back |

**Reasoning for the split:**

- Decision phases (AMBULANCE, BED, HOSPITAL_LIST, HOSPITAL_DETAIL) are exploratory â€” the user is browsing, not committed. Back via map chrome is natural and low-risk. The sheet header's close button already exists; the chrome back is an additive affordance that mirrors Apple Maps behavior.
...
- `COMMIT_PAYMENT` during `WAITING_APPROVAL` must never offer any back path â€” this is the PT-6 patient-safety rule. The chrome button must not bypass it.
...
The `onBack` prop for authenticated decision phases should call `onCloseDecisionPhase` (or equivalent) â€” which routes back to EXPLORE_INTENT via `buildExploreIntentSheetView`. This matches the existing `closeCommitDetails` / `closeCommitPayment` pattern in `useMapCommitFlow.js` â€” use `buildSourceReturnSheetView` or `buildExploreIntentSheetView`.
...
```js
// Cleaner approach â€” no new prop: caller controls visible, component just calls the right handler
...
- `MapScreen.jsx` stays thin â€” phase-aware `visible` logic is two lines, not business logic
- No new chrome component â€” extends existing `MapTopLeftControl` with one new boolean prop
- No `useEffect` for visibility â€” derived inline from `sheetPhase` (REFACTORING_GUARDRAILS Â§useEffect Decision Tree: "derived value â†’ inline const")
- `WAITING_APPROVAL` lock not bypassed â€” `COMMIT_PAYMENT` is explicitly excluded from `isDecisionPhase`
- Haptic is already present on `handlePress` via `Haptics.selectionAsync()` â€” no change needed
...
**Defects addressed:** Issue 1 (triage pre-selection), Issue 2 (HALF snap layout), Issue 3 (saved transport card), Issue 6 (transport hierarchy), Issue 7 (transport truncation), MapTopLeftControl back-nav (Â§1.5)  
**Files touched:**
...
| `useMapCommitTriageController.js` | Add `isFreshSession` guard â€” empty draft on fresh open, restore only on resume |
| `mapCommitTriage.helpers.js` | Add `buildFreshTriageDraft()` helper â€” returns empty dict keyed to step IDs |
| `MapBedDecisionStageBase.jsx` | In HALF snap: RouteCard â†’ compact single-row strip (not full removal); SavedTransportCard â†’ status strip |
...
| `mapAmbulanceDecision.content.js` | Audit title copy â€” task language `"Select Transportation"` |
| `mapBedDecision.content.js` | Audit sheet title copy â€” `"Choose Your Room"` |

**Four-track:**

| Track | Scope |
|---|---|
| State management | `useMapCommitTriageController.js` â€” initialization path change only. No new state layer. `isDecisionPhase` derived inline (not `useEffect`) â€” per REFACTORING_GUARDRAILS Â§useEffect Decision Tree: derived value â†’ `useMemo` or inline const. |
...
| Documentation | PULLBACK NOTE on every structural change (format per Â§6). This file Â§3 pass log updated post-commit. |

**Guardrails compliance for UX-A:**

- `MapScreen.jsx` remains thin â€” `isDecisionPhase` is two derived lines, not business logic (MAP_SCREEN_IMPLEMENTATION_RULES_V1 Â§3)
- `COMMIT_PAYMENT` excluded from `isDecisionPhase` â€” WAITING_APPROVAL lock cannot be bypassed via chrome (TRACKING_SHEET_LEARNINGS defect class 2.14)
- No `useEffect` for phase-aware visibility â€” derived from `sheetPhase` prop already in scope
- No anonymous functions in props â€” `handleDecisionBack` extracted as `useCallback` in `MapScreen.jsx` (REFACTORING_BIBLE Commandment 2)
- Triage `isFreshSession` guard: no `useEffect` introduced â€” guard is applied during `useMemo` initialization, not as a side effect

**Invariants (must not change):**

- `MapBedDecisionSavedTransportCard` full-card kept and rendered in EXPANDED snap â€” no deletion
...
| File | Change |
|---|---|
| `mapHospitalDetail.helpers.js` | Reorder badge priority: capability first â†’ trust/quality last. Add `accessibilityHint` per badge. `"Standard"` badge â†’ muted styling. |
...
| `MapCommitDetailsStageParts.jsx` | CTA state machine: `idle â†’ pressed â†’ requesting â†’ ready â†’ failed` |
...
| Track | Scope |
|---|---|
| State management | `useMapCommitDetailsController.js` â€” OTP call moved earlier in the press handler. No new state layer. |
...
- Badge reorder is a pure helper function change â€” no component restructure, no state (REFACTORING_BIBLE: business logic in helpers not components)
- OTP API early trigger: verify it does not violate the `useEffect` decision tree â€” the call is a side effect on press (timer/network), so a direct call in the event handler is correct. Do not wrap it in `useEffect`
- Phase transition: use existing `MapPhaseTransitionView` â€” do not create a new animated wrapper (TRACKING_SHEET_LEARNINGS defect class 2.1: modal/surface gated on transient parent)
- No new files created unless a StageParts file crosses 950 lines

**Invariants:**

- Badge rendering component unchanged â€” only helper output order changes
...
| `MapCommitPaymentStageParts.jsx` | Add `FINALIZING_DISPATCH` UI surface â€” non-retryable "Payment sent â€” confirming dispatch" |
| `MapCommitPaymentStageParts.jsx` | Wallet row: disabled state with balance caption `"Balance $X.XX â€” not enough"` |
| `mapCommitPayment.content.js` | Audit CTA label constants â€” task language: `"Confirm & Dispatch"` / `"Pay $X.XX"` |
| `useMapCommitPaymentController.js` | Stable display ID: `useRef` per mount instead of `Math.random()` per submit (PT-7) |
| `mapCommitPayment.helpers.js` | Remove `"8 mins"` fallback ETA â€” return `null` (PT-11) |
| `MapCommitPaymentStageBase.jsx` | ETA display: handle `null` â†’ show `"Calculatingâ€¦"` or omit pill |

**Four-track:**

| Track | Scope |
|---|---|
| State management | No layer changes. `FINALIZING_DISPATCH` already exists as a `submissionState.kind` constant â€” only the UI surface is new. |
...
- Sticky footer CTA: `EntryActionButton` stays the same primitive. Prop threading (`canConfirm`, `primaryActionTitle`, `onSubmit`) goes up to StageBase â€” this is correct layer ownership (view composes, controller owns handlers). Not a state layer change.
- `FINALIZING_DISPATCH` UI: check TRACKING_SHEET_LEARNINGS defect class 2.1 â€” the `FINALIZING_DISPATCH` surface must be rendered at the StageBase level, not inside a child card that can unmount
- Wallet disabled state: render the option with `disabled` styling + caption. Do not conditionally remove it from the list. Per REFACTORING_GUARDRAILS Â§Loading State Rule: preserve the layout shell, show pending/disabled state, never blank
- PT-7 display ID: `useRef` initialized once on mount â€” not a `useEffect` (derived constant via ref, per useEffect decision tree: "Y is a ref â†’ assign inline")
- PT-11 ETA `null`: removing the `"8 mins"` string is the only change. The tracking sheet ETA display null-path must be verified before commit â€” no fabricated data in emergency context (Apple HIG trust standard)
...
- CTA sticky footer uses the same `EntryActionButton` component â€” no new button primitive
...
**Defects addressed:** Issue 9 (navigation reset), C-1/PT-B (TanStack Query), C-2/CV-2 (`isSubmitting` removal), C-3/PT-4 (atomic store transition), C-4/PT-3 (`commitFlow` â†’ Jotai)  
**Files touched:**
...
| `emergencyTripStore.js` | Add `transitionPendingToActive(trip)` action â€” atomic single write |
| `useMapCommitPaymentController.js` | Replace `isSubmitting` boolean â€” derive all UI state from `submissionState.kind` |
| `MapCommitPaymentStageBase.jsx` | Update consumers of `isSubmitting` â€” derive from `submissionState.kind` |
...
| Track | Scope |
|---|---|
| State management | `commitFlow` â†’ Jotai (session-ephemeral). Payment data â†’ TanStack Query (layer 2). `isSubmitting` removed (single source of truth). `transitionPendingToActive` Zustand action added. |
| UI quality | No visual changes. Data already correct â€” this pass removes the loading churn that the user perceives as flicker. |
...
- State layer assignments per REFACTORING_GUARDRAILS Â§Canonical Layers:
  - `commitFlow` â†’ Jotai (ephemeral UI state, not persistent client state) âœ“
  - Payment methods list â†’ TanStack Query (server cache) âœ“
  - `isSubmitting` â†’ remove (derived from `submissionState.kind`, per useEffect decision tree: "machine state â†’ Jotai atom or XState, not boolean useState")
  - `transitionPendingToActive` â†’ Zustand action (persistent client state) âœ“
...
- Stash adoption: read each stash file against current code before adopting â€” never drop logic silently (TRACKING_SHEET_LEARNINGS Â§1.4 "never bundle gains")
- `isSubmitting` removal: grep all consumers before removing â€” blast radius audit is mandatory, not optional
- TanStack Query: `QueryClient.invalidateQueries` must be wired before `refreshPaymentMethodSnapshot` is removed â€” never leave a gap window where methods list is stale after a new card is added
- One sub-step per commit. Do not batch stash adoptions. Each adoption is its own checkpoint.

**Invariants (CRITICAL â€” high blast-radius pass):**
...
- TanStack Query adoption: must preserve wallet eligibility filter (`walletBalance >= checkoutTotal`) â€” this is a safety check.
...
### UX-E â€” LocationSheet + Mini Profile (Location Passes Dependent)
...
**Prerequisites:**
- LocationSheet decision tree (search â†’ candidate â†’ save category â†’ save details â†’ pickup commit) is stable
- LocationSheet has a single owner (not multiple modals)
- Location passes have shipped

**When unblocked, scope:**
- Mini profile: add `"Address & Location"` row â†’ opens LocationSheet with `sourcePhase: "miniProfile"` metadata
- LocationSheet close: when `sourcePhase === "miniProfile"`, return to mini profile (not explore intent)
- No new address management surface â€” LocationSheet is the sole owner
...
UX-C (payment polish)
  â†“
UX-D (state architecture)    â† highest blast radius, do last
  â†“
UX-E (location â€” blocked)    â† unblocks after Location passes
```
...
|---|---|---|---|
| `MapBedDecisionStageParts.jsx` | 899 | max 450 (Complex Feature Component) | ðŸš¨ Exceeds â€” flag responsibility leakage |
| `MapAmbulanceDecisionStageParts.jsx` | 746 | max 450 | ðŸš¨ Exceeds |
| `MapCommitPaymentStageParts.jsx` | 898 | max 450 | ðŸš¨ Exceeds |
| `useMapCommitTriageController.js` | 603 | max 300 (Hook) | ðŸš¨ Exceeds |
| `MapBedDecisionStageBase.jsx` | 467 | max 500 (Screen file) | âš ï¸ Near limit |
| `mapCommitPayment.transaction.js` | 166 | max 250 (State file) | âœ… |
...
1. **Behaviour parity check** â€” every component that existed before still renders the same data; only layout/order/copy changed
2. **HALF snap visual test** â€” bed decision sheet with `careIntent === "both"`: Hero + compact RouteStrip + StatusStrip + SwitchRow visible; no full RouteCard
3. **Triage fresh open** â€” fresh booking (no prior triageDraft): all options neutral; step 0 open; CTA disabled
4. **Triage resume** â€” return to triage with matching requestId + triageDraft: prior answers restored; step restored
5. **Payment CTA** â€” in HALF snap on non-sidebar layout: CTA visible without scrolling
6. **WAITING_APPROVAL** â€” re-tap CTA during approval wait: CTA locked (no dismiss) â€” this is the PT-C regression guard
7. **MapTopLeftControl back-nav** â€” authenticated user in AMBULANCE_DECISION: back chevron visible, tap returns to EXPLORE_INTENT. In COMMIT_PAYMENT: control is hidden entirely. In EXPLORE_INTENT: avatar visible, opens profile.
8. **Unauthenticated back-nav unchanged** â€” unauthenticated user in EXPLORE_INTENT: back chevron â†’ `/(auth)/`. Regression guard for existing behaviour.
9. **useEffect audit** â€” any new `useEffect` added in the pass must be justified against the decision tree in REFACTORING_GUARDRAILS. If it derives a value, convert to `useMemo` or inline const before merging.
10. **Line count** â€” no file introduced that exceeds its architecture target without documented justification and a noted extraction plan
...
- Post-commit: update Â§3 Pass Log in this document with actual files changed and test results

Done when:
- UX-A through UX-D are shipped and pass verification protocol
- All `ðŸ”´ Open` issues in Â§1 are moved to âœ…
