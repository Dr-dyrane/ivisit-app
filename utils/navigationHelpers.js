...
export const ROUTES = {
  ...
  STACK_NOTIFICATION_DETAILS: (id) =>
    `/(user)/(stacks)/notification-details?id=${String(id)}`,
  ...
  STACK_EMERGENCY_REQUEST_AMBULANCE:
    "/(user)/(stacks)/emergency/request-ambulance",
...
export function navigateToNotifications({ router, filter, method = "push" }) {
  ...
  nav(router, method, {
    pathname: ROUTES.STACK_NOTIFICATIONS,
    params: { filter: v },
  });
}
...
export function navigateToHelpSupport({ router, ticketId, method = "push" }) {
  if (ticketId) {
    nav(router, method, {
      pathname: "/(user)/(stacks)/help-support",
      params: { ticketId },
    });
    return;
  }
...
export function navigateToNotificationDetails({
  router,
  notificationId,
  method = "push",
}) {
