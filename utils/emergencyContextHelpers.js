...
export const AMBULANCE_LIVE_TRACK_STATUSES = new Set(["accepted", "in_progress", "arrived"]);
export const TELEMETRY_STALE_THRESHOLD_MS = 30000;
export const TELEMETRY_LOST_THRESHOLD_MS = 120000;
export const REALTIME_RECOVERY_STATUSES = new Set(["CHANNEL_ERROR", "TIMED_OUT", "CLOSED"]);
...
export const normalizeRouteCoordinates = (route) => {
	...
	return route.map((p) => normalizeCoordinate(p)).filter((p) => isValidCoordinate(p));
};
...
export const interpolateRoutePosition = (routeCoordinates, progressRatio) => {
	...
	const segIdx = Math.min(routeCoordinates.length - 2, Math.floor(segmentProgress));
