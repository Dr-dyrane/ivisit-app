...
export default function InsuranceScreen() {
	...
	const canSave = () => {
		...
		if (step === 2) return true; // Images are optional but encouraged
		return true;
	};
	...
	const transitionStep = (newStep) => {
		if (newStep < 0 || newStep > 2) return; // 3 steps: 0=Provider, 1=Policy, 2=Images
		LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
		...
		try {
			...
			if (status !== 'granted') {
				Alert.alert(
					...
					"To scan your insurance card, we need access to your camera. Please enable camera permissions in your device settings.",
					[
						{ text: "Cancel", style: "cancel" },
			...
			const result = await ImagePicker.launchCameraAsync({
				...
				allowsEditing: true,
				aspect: [16, 10], // Card aspect ratio
				quality: 0.8,
			});
			...
			if (!result.canceled) {
				...
				if (scanResult.success) {
					...
					setFormData(prev => ({
						...
						front_image_url: uri // Store the scanned image
					}));

					Alert.alert(
						"Card Scanned Successfully",
						"We've extracted your insurance details. Please review and confirm the information.",
						[{ text: "OK" }]
					);
				} else {
					// Still store the image even if OCR failed
					setFormData(prev => ({
						...prev,
						front_image_url: uri
					}));
			...
			Alert.alert(
				"Scan Failed",
				"Unable to scan the insurance card. Please try again or enter details manually.",
				[{ text: "OK" }]
			);
		} finally {
			setIsScanning(false);
		...
		try {
			...
			if (status !== 'granted') {
				Alert.alert(
					...
					"To select images from your library, we need access to your photos. Please enable photo library permissions in your device settings.",
					[
						{ text: "Cancel", style: "cancel" },
						{ text: "Open Settings", onPress: () => ImagePicker.openSettingsAsync() }
					]
			...
			if (!result.canceled) {
				...
				// Don't trigger OCR for front image since we have dedicated scan button
				Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
			}
		} catch (error) {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			// Check if front image is a local URI (needs upload)
			...
			// Check if back image is a local URI (needs upload)
					...
					{step === 2 && (
						<View style={{ gap: 24 }}>
							<Text style={{
								...
								Upload photos of your insurance card to speed up verification.
							</Text>
							...
							<View style={{ flexDirection: 'row', gap: 16 }}>
								...
								<TouchableOpacity
									...
									style={{
										flex: 1,
										aspectRatio: 1, // Square-ish container
										backgroundColor: isDarkMode ? "rgba(255,255,255,0.05)" : "#F1F5F9",
										borderRadius: 20,
										alignItems: "center",
										justifyContent: "center",
								...
								>
									{formData.front_image_url ? (
										...
										<View style={{ alignItems: 'center', gap: 8 }}>
											...
											<Text style={{ fontSize: 12, fontWeight: '700', color: colors.textMuted }}>FRONT</Text>
										</View>
									)}
								</TouchableOpacity>
								...
								>
									{formData.back_image_url ? (
										...
										<View style={{ alignItems: 'center', gap: 8 }}>
											...
											<Text style={{ fontSize: 12, fontWeight: '700', color: colors.textMuted }}>BACK</Text>
										</View>
									)}
								</TouchableOpacity>
							</View>
