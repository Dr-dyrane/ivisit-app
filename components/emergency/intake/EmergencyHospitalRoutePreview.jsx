...
function getRegionForCoordinates(coordinates, bottomPadding = 0) {
  ...
  const routeIsMostlyHorizontal =
    rawLongitudeSpan > Math.max(rawLatitudeSpan, 0.0001) * 1.2;
  ...
  const latitudeSpan = Math.max(
    0.0058,
    routeIsMostlyHorizontal ? cameraDiameter * 0.88 : cameraDiameter,
  );
  const longitudeSpan = Math.max(
    0.0058,
    routeIsMostlyHorizontal ? cameraDiameter : cameraDiameter * 0.92,
  );
  ...
  const verticalBias = routeIsMostlyHorizontal
    ? 0.18 + bottomPadBias
    : 0.15 + bottomPadBias * 0.94;

  return {
    latitude:
      (minLatitude + maxLatitude) / 2 - effectiveVerticalSpan * verticalBias,
...
export default function EmergencyHospitalRoutePreview({
  ...
  const normalizedRouteCoordinates = useMemo(
    () =>
      Array.isArray(routeCoordinates)
        ? routeCoordinates.map((point) => toCoordinate(point)).filter(Boolean)
        : [],
    [routeCoordinates],
  );
  ...
  const markerRenderPulseKey = useMemo(
    () =>
      [
        `origin@${toMarkerPulseCoordinateKey(originCoordinate)}`,
        `hospital@${toMarkerPulseCoordinateKey(hospitalCoordinate)}`,
      ].join("|"),
    [hospitalCoordinate, originCoordinate],
  );
  ...
  useEffect(() => {
    ...
    const fit = () => {
      ...
      try {
        ...
        console.warn(
          "[EmergencyHospitalRoutePreview] Skipping programmatic fit:",
          error?.message || error,
        );
      }
    };
