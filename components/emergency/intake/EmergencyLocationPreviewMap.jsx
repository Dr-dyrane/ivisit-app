...
function toMarkerPulseCoordinateKey(coordinate) {
  ...
  return `${Number(coordinate.latitude).toFixed(5)}:${Number(coordinate.longitude).toFixed(5)}`;
}
...
function sortHospitalsForPreview(hospitals, selectedHospitalId) {
  return hospitals
    ...
    .sort((left, right) => {
      const leftSelected =
        selectedHospitalId && left?.id === selectedHospitalId ? 1 : 0;
      const rightSelected =
        selectedHospitalId && right?.id === selectedHospitalId ? 1 : 0;
      if (leftSelected !== rightSelected) {
        return rightSelected - leftSelected;
      }
      const leftDistance = Number(
        left?.distanceKm ?? left?.distance ?? Number.MAX_SAFE_INTEGER,
      );
      const rightDistance = Number(
        right?.distanceKm ?? right?.distance ?? Number.MAX_SAFE_INTEGER,
      );
...
function getHorizontalOcclusionBias(leftPanelWidth = 0, screenWidth = 0) {
  if (
    !Number.isFinite(leftPanelWidth) ||
    leftPanelWidth <= 0 ||
...
function buildRegionForPoints(
  ...
  const routeIsMostlyHorizontal =
    rawLongitudeSpan > Math.max(rawLatitudeSpan, 0.0001) * 1.12;
  ...
  const sheetBias = Math.min(
    0.18,
    Math.max(0.05, (Number(bottomSheetHeight) || 0) / 1500),
  );
  const normalizedBias = wideViewport
    ? Math.min(0.09, Math.max(0.015, sheetBias * 0.66))
    : Math.min(0.12, Math.max(0.02, sheetBias));
  const headerBias = Math.min(
    0.055,
    Math.max(0, (Number(headerOcclusionHeight) || 0) / 3200),
  );
  const horizontalBias = getHorizontalOcclusionBias(
    leftPanelWidth,
    screenWidth,
  );
  ...
  return {
    latitude:
      (minLat + maxLat) / 2 + latitudeDelta * (normalizedBias + headerBias),
...
function buildUserCenteredRegion(
  coordinate,
  leftPanelWidth = 0,
  screenWidth = 0,
) {
...
function getWebRoutePanOffset(
  ...
  return Math.round(
    (Number(padding.bottom || 0) - Number(padding.top || 0)) / 2,
  );
}
...
export default function EmergencyLocationPreviewMap({
  ...
  const occlusionSignatureRef = useRef(
    `${Math.round(bottomSheetHeight)}|${Math.round(leftPanelWidth)}`,
  );
  ...
  const serviceMarkerOpacity =
    telemetryState === "lost" ? 0.62 : telemetryState === "stale" ? 0.86 : 1;
  ...
  const selectedHospital = useMemo(() => {
    ...
    return (
      visibleHospitals.find(
        (hospital) => hospital?.id === selectedHospitalId,
      ) || visibleHospitals[0]
    );
  }, [selectedHospitalId, visibleHospitals]);
  const selectedHospitalCoordinate = useMemo(
    () => toCoordinate(selectedHospital),
    [selectedHospital],
  );
  ...
  const nearbyRadiusKm = useMemo(() => {
    const nearestDistance = Number(
      selectedHospital?.distanceKm ?? visibleHospitals?.[0]?.distanceKm,
    );
  ...
  const hasActiveTrackingTimeline = useMemo(() => {
    ...
    const rawStartedAt =
      trackingTimeline?.startedAtMs ?? trackingTimeline?.startedAt;
  ...
  const initialAnimationProgress = useMemo(() => {
    const etaSeconds = Number(
      trackingTimeline?.etaSeconds ?? resolvedRouteInfo.durationSec,
    );
    const rawStartedAt =
      trackingTimeline?.startedAtMs ?? trackingTimeline?.startedAt;
    const startedAtMs = Number.isFinite(rawStartedAt)
    ...
    () =>
      [
        ...visibleHospitals.map((hospital, index) => {
          ...
          return `${hospital?.id || `hospital-${index}`}@${toMarkerPulseCoordinateKey(coordinate)}`;
        }),
        `selected@${toMarkerPulseCoordinateKey(selectedHospitalCoordinate)}`,
        ...
        {visibleHospitals.map((hospital, index) => {
          ...
          return (
            <Marker
              ...
              onPress={
                onHospitalPress ? () => onHospitalPress(hospital) : undefined
              }
            />
          );
        })}
