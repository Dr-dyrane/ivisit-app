...
function MiniProfileShortcutRow({ row, isLast, colors, layout }) {
  const hasBadge =
    row.badge !== null && row.badge !== undefined && row.badge !== "";
  const hasSubtitle = Boolean(row.subtitle);
  const rowMinHeight = hasSubtitle
    ? layout.row.minHeight + 12
    : layout.row.minHeight;
          ...
          {hasSubtitle ? (
            <Text
              style={[
                styles.rowSubtitle,
                {
                  color: row.subtitleColor || colors.muted,
                  fontSize: row.subtitleSize || 13,
                  lineHeight: row.subtitleLineHeight || 18,
                  fontWeight: row.subtitleWeight || "400",
                },
              ]}
              numberOfLines={row.subtitleNumberOfLines || 2}
            >
              {row.subtitle}
            </Text>
          ) : null}
