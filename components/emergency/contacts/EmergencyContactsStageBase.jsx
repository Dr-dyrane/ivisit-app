...
import { createEmergencyContactsTheme } from "./emergencyContacts.theme";
import { computeEmergencyContactsSidebarLayout } from "./emergencyContactsSidebarLayout";

// PULLBACK NOTE: EmergencyContacts StageBase mirrors PaymentStageBase at MD+.
// OLD: One animated ScrollView at every width, with a late two-column split inside content.
// NEW: Shared stack viewport contract decides between compact scroll and full-viewport sidebar overlay.
// REASON: Preserve mobile behavior while giving tablet/desktop the same shell semantics as payment.

export default function EmergencyContactsStageBase({ isDarkMode, children }) {
  ...
  const theme = useMemo(
    () => createEmergencyContactsTheme({ isDarkMode }),
    [isDarkMode],
  );
