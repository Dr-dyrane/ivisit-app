...
export const BOOK_VISIT_SIDEBAR_HIG = Object.freeze({
...
export function getBookVisitSidebarGlassTokens({ isDarkMode }) {
  return {
...
export function computeBookVisitSidebarLayout({ width, surfaceConfig }) {
  ...
  const sidebarGutter =
    ...
    BOOK_VISIT_SIDEBAR_HIG.GUTTER_FALLBACK;

  const sidebarLeft = sidebarGutter;
  const sidebarInnerPadding =
    surfaceConfig.headerSideInset ||
    BOOK_VISIT_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_FALLBACK;
  const sidebarInnerPaddingHorizontal =
    BOOK_VISIT_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_HORIZONTAL;
...
export function computeBookVisitThirdColumnLayout({
  layout,
  viewportVariant,
  width = 0,
}) {
  ...
  const centerMinWidth =
    viewportVariant === STACK_VIEWPORT_VARIANTS.WEB_XL ? 640 : 700;
...
export function computeBookVisitHeaderClearance({
  surfaceConfig,
  insetsTop = 0,
}) {
  return (
    BOOK_VISIT_SIDEBAR_HIG.HEADER_ISLAND_HEIGHT +
    (surfaceConfig?.headerTopInset || 14) +
    insetsTop +
    BOOK_VISIT_SIDEBAR_HIG.HEADER_BREATHING
  );
}
