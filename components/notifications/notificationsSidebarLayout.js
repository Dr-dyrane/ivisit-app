...
export const NOTIFICATIONS_SIDEBAR_HIG = Object.freeze({
...
export function getNotificationsSidebarGlassTokens({ isDarkMode }) {
  return {
...
export function computeNotificationsSidebarLayout({ width, surfaceConfig }) {
  ...
  const sidebarGutter =
    ...
    NOTIFICATIONS_SIDEBAR_HIG.GUTTER_FALLBACK;

  const sidebarLeft = sidebarGutter;
  const sidebarInnerPadding =
    surfaceConfig.headerSideInset ??
    NOTIFICATIONS_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_FALLBACK;
  const sidebarInnerPaddingHorizontal =
    NOTIFICATIONS_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_HORIZONTAL;
...
export function computeNotificationsThirdColumnLayout({
  layout,
  viewportVariant,
  width = 0,
}) {
...
export function computeNotificationsHeaderClearance({
  surfaceConfig,
  insetsTop = 0,
}) {
  return (
    NOTIFICATIONS_SIDEBAR_HIG.HEADER_ISLAND_HEIGHT +
    (surfaceConfig?.headerTopInset ?? 14) +
    insetsTop +
    NOTIFICATIONS_SIDEBAR_HIG.HEADER_BREATHING
  );
}
