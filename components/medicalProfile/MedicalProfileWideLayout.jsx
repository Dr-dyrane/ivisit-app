...
import MedicalProfileActionIsland from "./MedicalProfileActionIsland";
...
import {
  computeMedicalProfileHeaderClearance,
  computeMedicalProfileThirdColumnLayout,
  getMedicalProfileSidebarGlassTokens,
  MEDICAL_PROFILE_SIDEBAR_HIG,
} from "./medicalProfileSidebarLayout";

// PULLBACK NOTE: MedicalProfile wide-screen variant follows the shared stack-shell pattern.
// Left island owns context, center owns the summary blades, and XL right island fills dead space with focused status and the edit action.

export default function MedicalProfileWideLayout({
  isDarkMode,
  theme,
  metrics,
  layout,
  ...
  const glass = useMemo(
    () => getMedicalProfileSidebarGlassTokens({ isDarkMode }),
    [isDarkMode],
  );
    ...
    <>
      ...
      >
        ...
        <View style={{ flexDirection: "column", gap: 1 }}>
          ...
          >
            {MEDICAL_PROFILE_SCREEN_COPY.screen.title}
          </Text>
        </View>
      </View>
      ...
      <BlurView
        ...
        style={{
          ...
          borderRadius: MEDICAL_PROFILE_SIDEBAR_HIG.SIDEBAR_CORNER_RADIUS,
      ...
      >
        <View style={{ gap: metrics.spacing.xs }}>
          ...
          >
            {MEDICAL_PROFILE_SCREEN_COPY.center.title}
          </Text>
        </View>
      ...
      {showThirdColumn ? (
        ...
        >
          <ScrollView
            ...
            showsVerticalScrollIndicator={false}
          >
            <MedicalProfileActionIsland
              theme={theme}
              metrics={metrics}
              completionLabel={model.completionLabel}
              criticalStatusLabel={model.criticalStatusLabel}
              historyStatusLabel={model.historyStatusLabel}
              notesStatusLabel={model.notesStatusLabel}
              syncNotice={model.syncNotice}
              onEditProfile={model.openEditor}
              loading={loading}
            />
