...
// PULLBACK NOTE: Profile wide-shell geometry mirrors the payment/emergency stack contract.
// Owns: sidebar/header/third-column math so the header pill and wide layout stay aligned.

export const PROFILE_SIDEBAR_HIG = Object.freeze({
...
export function getProfileSidebarGlassTokens({ isDarkMode }) {
  return {
...
export function computeProfileSidebarLayout({ width, surfaceConfig }) {
  ...
  const sidebarGutter =
    ...
    PROFILE_SIDEBAR_HIG.GUTTER_FALLBACK;

  const sidebarLeft = sidebarGutter;
  const sidebarInnerPadding =
    surfaceConfig.headerSideInset ??
    PROFILE_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_FALLBACK;
  const sidebarInnerPaddingHorizontal =
    PROFILE_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_HORIZONTAL;
...
export function computeProfileThirdColumnLayout({ layout, viewportVariant }) {
  const isXL =
...
export function computeProfileHeaderClearance({
  surfaceConfig,
  insetsTop = 0,
}) {
  return (
    PROFILE_SIDEBAR_HIG.HEADER_ISLAND_HEIGHT +
    (surfaceConfig?.headerTopInset ?? 14) +
    insetsTop +
    PROFILE_SIDEBAR_HIG.HEADER_BREATHING
  );
}
