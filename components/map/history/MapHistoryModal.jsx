...
function HistorySkeletonGroup({ theme, rowMetrics, containerRadius, rows = 3 }) {
	return (
...
export default function MapHistoryModal({
	...
	const [filter, setFilter] = useState(HISTORY_FILTER_KEYS.ALL);
	...
	const rowMetrics = useMemo(() => {
		...
		const orbSize = Math.max(40, Math.round(viewportMetrics.radius.card * 1.44));
		return {
			iconSize,
			orbSize,
			...
			subtitleLineHeight: Math.max(17, viewportMetrics.type.captionLineHeight + 1),
			chevronSize: 16,
		};
	}, [viewportMetrics]);
	...
	const emptyMetrics = useMemo(
		() => ({
			...
			actionPaddingHorizontal: Math.max(16, viewportMetrics.modal.contentPadding - 2),
			actionPaddingVertical: 14,
			actionTextSize: Math.max(15, viewportMetrics.type.body),
			actionTextLineHeight: Math.max(20, viewportMetrics.type.bodyLineHeight - 4),
		}),
		[viewportMetrics],
	);
	...
	const bottomAction = (() => {
		if (typeof onBookVisit === "function") {
			return { label: HISTORY_MODAL_BOTTOM_ACTION_COPY.bookVisit, onPress: onBookVisit };
		}
		if (typeof onChooseCare === "function") {
			return { label: HISTORY_MODAL_BOTTOM_ACTION_COPY.chooseCare, onPress: onChooseCare };
		}
		return null;
	})();
					...
					isAllFilter ? (
						<View style={historyModalStyles.groupStack}>
							{groupedBuckets.map((group) => (
								<MapHistoryGroup
									key={group.key}
									label={group.label || REQUEST_HISTORY_GROUP_LABELS[group.key] || null}
									items={group.items}
