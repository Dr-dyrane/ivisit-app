...
export function useMapCommitPaymentController({
  ...
  const refreshPaymentMethodSnapshot = useCallback(
    async ({ preferredMethod = null } = {}) => {
      ...
      try {
        ...
        const currentMethod =
          preferredMethod || selectedPaymentMethodRef.current || null;
        ...
        const defaultMethod =
          (demoCashOnly
            ? availableMethods.find((method) => method.is_cash)
            : null) ||
          selectedMatch ||
          cachedMatch ||
          dbDefault ||
        ...
        if (paymentMethodsRefreshRef.current !== refreshId) return;
        console.warn(
          "[MapCommitPayment] payment method refresh failed:",
          error,
        );
        setSelectedPaymentMethod(null);
        setPaymentMethodsSnapshotReady(true);
  ...
  const canDismissStatusState = isCommitPaymentDismissibleState(
    submissionState,
    {
      isSubmitting,
    },
