...
export default function useMapCommitTriageController({
  ...
  const { hospitals, selectedSpecialty } = useEmergency();
  ...
  const activeAmbulanceTrip = useEmergencyTripStore(
    (s) => s.activeAmbulanceTrip,
  );
  ...
  const patchActiveAmbulanceTrip = useEmergencyTripStore(
    (s) => s.patchActiveAmbulanceTrip,
  );
  const patchActiveBedBooking = useEmergencyTripStore(
    (s) => s.patchActiveBedBooking,
  );
  ...
  useEffect(() => {
    ...
    setActiveStepId(
      payload?.activeStep ||
        getFirstOpenCommitTriageStepId(steps, initialDraft),
    );
  ...
  const headerSubtitle = [hospitalName, selectionLabel]
    .filter(Boolean)
    .join(" · ");
    ...
    (nextDraft, nextStepId, nextShowExtendedComplaints) => {
      const nextSnapshot =
        hasMeaningfulTriageDraftData(nextDraft) && nextStepId
          ? buildCommitTriageSnapshot({
              ...
              hospitalId: hospital?.id || null,
              serviceType:
                payload?.careIntent === "bed" && !transport
                  ? "bed"
                  : "ambulance",
              specialty: selectedSpecialty || null,
              medicalProfile,
      ...
      setCommitFlow?.({
        ...
        triageSnapshot: nextSnapshot
          ? {
              ...nextSnapshot,
              progress: buildCommitTriageProgressMeta(
                steps,
                nextDraft,
                nextStepId,
              ),
  ...
  const handleSelectOption = useCallback(
    (optionValue) => {
      ...
      const nextDraft = applyCommitTriageSelection(
        activeStep,
        optionValue,
        draft,
      );
  ...
  return {
    ...
    topSlotTitle:
      activeStep?.headerTitle || MAP_COMMIT_TRIAGE_COPY.HEADER_TITLE,
    topSlotSubtitle: headerSubtitle
      ? `For ${headerSubtitle}`
      : "For this request",
    orbSize,
    orbRadius,
