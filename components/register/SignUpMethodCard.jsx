...
export default function SignUpMethodCard({ onSelect }) {
  ...
  // Animate button list once
  const listAnim = useRef(new Animated.Value(30)).current;
  const listOpacity = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    Animated.parallel([
      Animated.spring(listAnim, { toValue: 0, tension: 40, friction: 8, useNativeDriver: true }),
    ...
    return (
      <Animated.View style={{ transform: [{ scale }] }}>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ color: colors.text, fontSize: 19, fontWeight: "900", letterSpacing: -0.5 }}>
              {label}
            </Text>
            <Text style={{ color: colors.subtitle, fontSize: 14, marginTop: 2 }}>
              {description}
            </Text>
          </View>
