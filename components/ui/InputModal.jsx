...
export default function InputModal({
  ...
  const stackSurfaceConfig = useMemo(
    () => getStackViewportSurfaceConfig(viewportVariant),
    [viewportVariant],
  );
  const resolvedModalMaxWidth =
    modalMaxWidth ?? stackSurfaceConfig.modalMaxWidth ?? null;
  const resolvedModalMaxHeightRatio =
    modalMaxHeightRatio ?? stackSurfaceConfig.modalMaxHeightRatio ?? 0.85;
  const resolvedModalPresentationMode =
    modalPresentationMode ??
    stackSurfaceConfig.modalPresentationMode ??
    "centered-modal";
  const isBottomSheet = resolvedModalPresentationMode === "bottom-sheet";
  const canDismiss = allowDismissWhileLoading || !loading;
  const horizontalInset = stackSurfaceConfig.contentHorizontalPadding ?? 12;
  ...
  const {
    ...
    maxHeightPercentage: resolvedModalMaxHeightRatio,
  });
          ...
          <View
            style={[
              styles.centeredContainer,
              {
                ...
                paddingTop: horizontalInset,
                paddingBottom: isBottomSheet ? 0 : horizontalInset,
              },
            ]}
          >
            <View
              style={[
                styles.modalContent,
                isBottomSheet ? styles.bottomSheetContent : null,
                {
                  backgroundColor: colors.bg,
                  maxHeight: modalHeight,
                  maxWidth: resolvedModalMaxWidth || undefined,
                  borderRadius: isBottomSheet ? 28 : 32,
                },
              ]}
            >
              {/* Header */}
