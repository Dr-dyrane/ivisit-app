...
export const HELP_SUPPORT_SIDEBAR_HIG = Object.freeze({
...
export function getHelpSupportSidebarGlassTokens({ isDarkMode }) {
  return {
...
export function computeHelpSupportSidebarLayout({ width, surfaceConfig }) {
  ...
  const sidebarGutter =
    ...
    HELP_SUPPORT_SIDEBAR_HIG.GUTTER_FALLBACK;

  const sidebarLeft = sidebarGutter;
  const sidebarInnerPadding =
    surfaceConfig.headerSideInset ??
    HELP_SUPPORT_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_FALLBACK;
  const sidebarInnerPaddingHorizontal =
    HELP_SUPPORT_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_HORIZONTAL;
...
export function computeHelpSupportThirdColumnLayout({
  layout,
  viewportVariant,
  width = 0,
}) {
...
export function computeHelpSupportHeaderClearance({
  surfaceConfig,
  insetsTop = 0,
}) {
  return (
    HELP_SUPPORT_SIDEBAR_HIG.HEADER_ISLAND_HEIGHT +
    (surfaceConfig?.headerTopInset ?? 14) +
    insetsTop +
    HELP_SUPPORT_SIDEBAR_HIG.HEADER_BREATHING
  );
}
