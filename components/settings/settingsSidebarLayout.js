...
// PULLBACK NOTE: Settings wide-shell geometry mirrors the shared stack sidebar pattern.
// It keeps header placement, left context island, and optional XL action island aligned.

export const SETTINGS_SIDEBAR_HIG = Object.freeze({
...
export function getSettingsSidebarGlassTokens({ isDarkMode }) {
  return {
...
export function computeSettingsSidebarLayout({ width, surfaceConfig }) {
  ...
  const sidebarGutter =
    ...
    SETTINGS_SIDEBAR_HIG.GUTTER_FALLBACK;

  const sidebarLeft = sidebarGutter;
  const sidebarInnerPadding =
    surfaceConfig.headerSideInset ??
    SETTINGS_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_FALLBACK;
  const sidebarInnerPaddingHorizontal =
    SETTINGS_SIDEBAR_HIG.SIDEBAR_INNER_PADDING_HORIZONTAL;
...
export function computeSettingsThirdColumnLayout({ layout, viewportVariant }) {
  const isXL =
...
export function computeSettingsHeaderClearance({
  surfaceConfig,
  insetsTop = 0,
}) {
  return (
    SETTINGS_SIDEBAR_HIG.HEADER_ISLAND_HEIGHT +
    (surfaceConfig?.headerTopInset ?? 14) +
    insetsTop +
    SETTINGS_SIDEBAR_HIG.HEADER_BREATHING
  );
}
