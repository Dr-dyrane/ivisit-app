...
const toGeometryPoint = (latitude, longitude) => `SRID=4326;POINT(${longitude} ${latitude})`;
...
const findAuthUserByEmail = (users, email) =>
  users.find((user) => String(user?.email || '').toLowerCase() === String(email || '').toLowerCase()) || null;
...
const syncProfileRole = async (userId, patch) => {
  const { error } = await supabase
    ...
    .eq('id', userId);
  if (error) throw new Error(`profile sync failed (${userId}): ${error.message}`);
};
...
const ensureOrgAdmin = async ({ authUsers, orgId, scope, hospitals, profiles }) => {
  const existingAdmin = profiles.find(
    (profile) => profile.organization_id === orgId && profile.role === 'org_admin'
  );
  if (existingAdmin) return existingAdmin.id;

  const email = toAuthEmail(scope, 'admin');
  const user = await ensureAuthUser(authUsers, email, toDisplayName('admin'), 'org_admin');
  ...
  const { error } = await supabase
    ...
    .in(
      'id',
      hospitals.filter((hospital) => hospital.organization_id === orgId).map((hospital) => hospital.id)
    );
...
const ensureHospitalStaffing = async ({
  ...
  if (!hasDoctor) {
    ...
    const doctorUser = await ensureAuthUser(authUsers, doctorEmail, toDisplayName('doctor', slot), 'provider');
  ...
  if (!hasAmbulance) {
    ...
    const driverUser = await ensureAuthUser(authUsers, driverEmail, toDisplayName('driver', slot), 'provider');
      ...
      .upsert(
        {
          ...
          location: toGeometryPoint(Number(hospital.latitude), Number(hospital.longitude)),
          updated_at: new Date().toISOString(),
        },
        { onConflict: 'profile_id', ignoreDuplicates: false }
      )
...
const main = async () => {
  const [
    ...
    supabase.from(TABLES.HOSPITALS).select('id,name,address,place_id,features,organization_id,latitude,longitude,org_admin_id'),
    supabase.from(TABLES.PROFILES).select('id,email,full_name,role,provider_type,organization_id,assigned_ambulance_id'),
    ...
    supabase.from(TABLES.PAYMENTS).select('id,organization_id,emergency_request_id,status'),
    supabase.from(TABLES.DOCTORS).select('id,profile_id,hospital_id'),
    supabase.from(TABLES.AMBULANCES).select('id,profile_id,organization_id,hospital_id'),
    supabase.from(TABLES.SERVICE_PRICING).select('id,hospital_id'),
    supabase.from(TABLES.ROOM_PRICING).select('id,hospital_id'),
    supabase.from(TABLES.EMERGENCY_REQUESTS).select('id,hospital_id,hospital_name'),
  ...
  const demoOrgs = organizations.filter((organization) =>
    /ivisit demo network|ivisit coverage network/i.test(String(organization.name || ''))
  );
  const demoOrgIds = new Set(demoOrgs.map((organization) => organization.id));
  const demoHospitals = hospitals.filter((hospital) => String(hospital.place_id || '').startsWith('demo:'));
  const activeDemoOrgIds = new Set(demoHospitals.map((hospital) => hospital.organization_id).filter(Boolean));

  const orphanDemoOrgs = demoOrgs.filter((organization) => !activeDemoOrgIds.has(organization.id));
  const hardDeleteOrgs = orphanDemoOrgs.filter(
    (organization) =>
      !payments.some((payment) => payment.organization_id === organization.id) &&
      !paymentMethods.some((paymentMethod) => paymentMethod.organization_id === organization.id)
  );

  const orphanOrgIds = new Set(orphanDemoOrgs.map((organization) => organization.id));
  ...
  const orphanDoctors = doctors.filter((doctor) => orphanProfileIds.includes(doctor.profile_id));
  const orphanAmbulances = ambulances.filter(
    (ambulance) =>
      orphanProfileIds.includes(ambulance.profile_id) || orphanOrgIds.has(ambulance.organization_id)
  );

  const dirtyRequests = requests.filter((request) => /\(demo\)/i.test(String(request.hospital_name || '')));
  ...
  const activeHospitalStatus = demoHospitals.map((hospital, index) => {
    ...
    return {
      ...
      hasAmbulance: ambulances.some((ambulance) => ambulance.hospital_id === hospital.id),
      hasServicePricing: servicePricing.some((row) => row.hospital_id === hospital.id),
      hasRoomPricing: roomPricing.some((row) => row.hospital_id === hospital.id),
    };
  });

  const repairTargets = activeHospitalStatus.filter(
    (item) => !item.hasDoctor || !item.hasAmbulance || !item.hasServicePricing || !item.hasRoomPricing
  );

  const activeAdminOrgIds = new Set(
    activeDemoProfiles.filter((profile) => profile.role === 'org_admin').map((profile) => profile.organization_id)
  );
  const missingAdminOrgs = [...activeDemoOrgIds].filter((orgId) => !activeAdminOrgIds.has(orgId));
  ...
  if (orphanAmbulances.length > 0) {
    const ambulanceIds = orphanAmbulances.map((ambulance) => ambulance.id);
    await supabase
      .from(TABLES.PROFILES)
      .update({ assigned_ambulance_id: null, updated_at: new Date().toISOString() })
      .in('assigned_ambulance_id', ambulanceIds);
    const { error } = await supabase.from(TABLES.AMBULANCES).delete().in('id', ambulanceIds);
    if (error) throw new Error(`orphan ambulance delete failed: ${error.message}`);
  }

  if (orphanDoctors.length > 0) {
    const { error } = await supabase.from(TABLES.DOCTORS).delete().in('id', orphanDoctors.map((doctor) => doctor.id));
    if (error) throw new Error(`orphan doctor delete failed: ${error.message}`);
  }

  if (orphanProfileIds.length > 0) {
    const { error } = await supabase.from(TABLES.PROFILES).delete().in('id', orphanProfileIds);
    if (error) throw new Error(`orphan profile delete failed: ${error.message}`);
    for (const profileId of orphanProfileIds) {
      try {
        await supabase.auth.admin.deleteUser(profileId);
      } catch (error) {
        throw new Error(`auth user delete failed (${profileId}): ${error.message || error}`);
      }
    }
  }

  if (hardDeleteOrgs.length > 0) {
    const hardDeleteOrgIds = hardDeleteOrgs.map((organization) => organization.id);
    const { error: walletError } = await supabase
      .from(TABLES.ORG_WALLETS)
      .delete()
      .in('organization_id', hardDeleteOrgIds);
    if (walletError) throw new Error(`organization wallet delete failed: ${walletError.message}`);

    const { error: orgError } = await supabase
      .from(TABLES.ORGANIZATIONS)
      .delete()
      .in('id', hardDeleteOrgIds);
    if (orgError) throw new Error(`organization delete failed: ${orgError.message}`);
  }

  for (const request of dirtyRequests) {
    const cleanedName = stripDemoSuffixes(request.hospital_name);
    if (normalizeText(cleanedName) === normalizeText(request.hospital_name)) continue;
    const { error } = await supabase
      .from(TABLES.EMERGENCY_REQUESTS)
      .update({ hospital_name: cleanedName, updated_at: new Date().toISOString() })
      .eq('id', request.id);
    if (error) throw new Error(`request name cleanup failed: ${error.message}`);
  }

  for (const orgId of missingAdminOrgs) {
    const orgHospitals = demoHospitals.filter((hospital) => hospital.organization_id === orgId);
    const scope = orgHospitals.length > 0 ? scopeForHospital(orgHospitals[0]) : 'unknown';
