...
const CONSOLE_SEARCH_SELECTIONS_SERVICE_FILE = path.join(
  CONSOLE_SRC,
  'services',
...
const OUT_FILE = path.join(OUT_DIR, 'search_selections_surface_field_guard_report.json');

const TABLE_NAME = 'search_selections';
const REQUIRED_RELATIONSHIPS = ['search_selections_user_id_fkey'];
const ALLOWED_SEARCH_SELECTIONS_REFERENCERS = new Set([
  'src/types/database.ts',
  'src/services/searchSelectionsService.js',
...
const EXPECTED_CREATE_FIELDS = ['user_id', 'query', 'result_type', 'result_id', 'source'];
const EXPECTED_UPDATE_FIELDS = ['query', 'result_type', 'result_id', 'source'];
...
function fail(message) {
  console.error(`[search-selections-surface-field-guard] FAIL: ${message}`);
  process.exit(1);
}
...
function compareTypeParity(appContent, consoleContent, violations) {
  ...
  if (!appTable) {
    pushViolation(violations, {
      ...
      message: 'app type contract missing search_selections table block.',
    });
    return null;
  }
  if (!consoleTable) {
    pushViolation(violations, {
      ...
      message: 'console type contract missing search_selections table block.',
    });
    return null;
  }
...
function validateReferenceBoundaries(violations) {
  ...
  for (const filePath of files) {
    ...
    if (!content.includes('search_selections')) continue;
    const relFile = normalizePath(path.relative(CONSOLE_ROOT, filePath));
    if (!ALLOWED_SEARCH_SELECTIONS_REFERENCERS.has(relFile)) {
      pushViolation(violations, {
        rule: 'search_selections_reference_outside_allowed_surfaces',
        file: relFile,
        line: 1,
        message: 'search_selections references must stay within approved type/service surfaces.',
      });
    }
  }
}
...
function validateQueryColumnsAndMutations(allowedColumns, violations) {
  const fromPattern = String.raw`(?:['"]search_selections['"]|TABLE_NAME|SEARCH_SELECTIONS_TABLE)`;
  ...
  for (const filePath of files) {
    ...
    while ((selectMatch = selectRegex.exec(content)) !== null) {
      ...
      if (invalidColumns.length > 0) {
        pushViolation(violations, {
          rule: 'non_schema_search_selections_select_columns',
          file: relFile,
          line,
          message: `search_selections select contains non-canonical columns: ${invalidColumns.join(', ')}`,
          snippet: selected,
        });
      }
    }
    ...
    while ((mutationMatch = mutationRegex.exec(content)) !== null) {
      ...
      if (!ALLOWED_MUTATION_OWNERS.has(relFile)) {
        pushViolation(violations, {
          rule: 'search_selections_mutation_outside_allowed_services',
          file: relFile,
          line,
...
function validateAllowlist(name, expectedFields, serviceContent, violations) {
  ...
  if (!match) {
    pushViolation(violations, {
      ...
      file: 'src/services/searchSelectionsService.js',
      message: `search selections service must declare ${name}.`,
    });
    return;
  }
...
function validateSearchSelectionsServiceContract(violations) {
  if (!fs.existsSync(CONSOLE_SEARCH_SELECTIONS_SERVICE_FILE)) {
    pushViolation(violations, {
      rule: 'search_selections_service_missing',
      file: 'src/services/searchSelectionsService.js',
      message: 'searchSelectionsService.js not found.',
    });
    return;
  }

  const content = fs.readFileSync(CONSOLE_SEARCH_SELECTIONS_SERVICE_FILE, 'utf8');
  validateAllowlist('SEARCH_SELECTION_CREATE_FIELDS', EXPECTED_CREATE_FIELDS, content, violations);
  validateAllowlist('SEARCH_SELECTION_UPDATE_FIELDS', EXPECTED_UPDATE_FIELDS, content, violations);

  const createFn = extractFunctionBlock(content, 'createSearchSelection');
  if (!createFn) {
    pushViolation(violations, {
      rule: 'create_search_selection_missing',
      file: 'src/services/searchSelectionsService.js',
      message: 'createSearchSelection function is missing.',
    });
  } else {
    if (!/payload\.created_at\s*=\s*new Date\(\)\.toISOString\(\);/.test(createFn.block)) {
      pushViolation(violations, {
        rule: 'create_search_selection_created_at_stamp_missing',
        file: 'src/services/searchSelectionsService.js',
        line: getLineNumber(content, createFn.start),
        message: 'createSearchSelection must stamp payload.created_at before insert.',
      });
    }
    if (!/payload\.source\s*=\s*payload\.source\s*\|\|\s*'search';/.test(createFn.block)) {
      pushViolation(violations, {
        rule: 'create_search_selection_source_default_missing',
        file: 'src/services/searchSelectionsService.js',
        line: getLineNumber(content, createFn.start),
        message: "createSearchSelection should default source to 'search'.",
      });
    }
  }

  const updateFn = extractFunctionBlock(content, 'updateSearchSelection');
  if (!updateFn) {
    pushViolation(violations, {
      rule: 'update_search_selection_missing',
      file: 'src/services/searchSelectionsService.js',
      message: 'updateSearchSelection function is missing.',
    });
  } else if (/user_id/.test(updateFn.block) && /payload\s*=/.test(updateFn.block)) {
    const updateAllowlistMatch = content.match(
      /const\s+SEARCH_SELECTION_UPDATE_FIELDS\s*=\s*\[([\s\S]*?)\];/
    );
    const updateFields = parseQuotedList(updateAllowlistMatch ? updateAllowlistMatch[1] : '');
    if (updateFields.includes('user_id')) {
      pushViolation(violations, {
        rule: 'update_search_selection_user_id_forbidden',
        file: 'src/services/searchSelectionsService.js',
        line: getLineNumber(content, updateFn.start),
        message: 'SEARCH_SELECTION_UPDATE_FIELDS must not include user_id reassignment.',
      });
    }
  }
}

function validateSearchServiceFallback(violations) {
  ...
  if (!/from\('search_selections'\)\.insert/.test(content)) {
    pushViolation(violations, {
      rule: 'search_service_selection_insert_missing',
      file: 'src/services/searchService.js',
      message: "searchService.recordSelection must insert into 'search_selections'.",
    });
  }
  if (!/isMissingRelationError\(error,\s*'search_selections'\)/.test(content)) {
    pushViolation(violations, {
      rule: 'search_service_selection_missing_relation_fallback_missing',
      file: 'src/services/searchService.js',
      message:
        "searchService.recordSelection must guard missing relation using isMissingRelationError(error, 'search_selections').",
    });
  }
  if (!/from\('search_events'\)\.insert/.test(content)) {
    pushViolation(violations, {
      rule: 'search_service_selection_fallback_insert_missing',
      file: 'src/services/searchService.js',
      message: "searchService.recordSelection must fallback to 'search_events' insert when relation is missing.",
    });
  }
}
...
function run() {
  ...
  validateSearchSelectionsServiceContract(violations);
  validateSearchServiceFallback(violations);

  const report = {
    generated_at: nowIso(),
    source: 'assert_search_selections_surface_field_guard.js',
  ...
  if (violations.length > 0) {
    console.error('[search-selections-surface-field-guard] FAIL: violations detected.');
    for (const violation of violations) {
      console.error(`- ${violation.file}:${violation.line} [${violation.rule}] ${violation.message}`);
    }
    console.error(`[search-selections-surface-field-guard] Report written: ${OUT_FILE}`);
    process.exit(1);
  }

  console.log(
    '[search-selections-surface-field-guard] PASS: no search_selections surface field violations detected.'
  );
  console.log(`[search-selections-surface-field-guard] Report written: ${OUT_FILE}`);
}
