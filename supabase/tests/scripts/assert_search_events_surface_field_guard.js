...
const CONSOLE_SEARCH_EVENTS_SERVICE_FILE = path.join(
  CONSOLE_SRC,
  'services',
  'searchEventsService.js'
);
const CONSOLE_SEARCH_ANALYTICS_SERVICE_FILE = path.join(
  CONSOLE_SRC,
  'services',
  'searchAnalyticsService.js'
);
const CONSOLE_SEARCH_SERVICE_FILE = path.join(CONSOLE_SRC, 'services', 'searchService.js');

const OUT_DIR = path.join(ROOT, 'supabase', 'tests', 'validation');
const OUT_FILE = path.join(OUT_DIR, 'search_events_surface_field_guard_report.json');

const TABLE_NAME = 'search_events';
const REQUIRED_RELATIONSHIPS = [];
const ALLOWED_SEARCH_EVENTS_REFERENCERS = new Set([
  'src/types/database.ts',
  'src/services/searchEventsService.js',
  'src/services/searchAnalyticsService.js',
  'src/services/searchService.js',
  'src/services/analyticsAutomationService.js',
]);
...
const EXPECTED_CREATE_FIELDS = ['query', 'source', 'selected_key', 'metadata'];
...
function fail(message) {
  console.error(`[search-events-surface-field-guard] FAIL: ${message}`);
  process.exit(1);
}
...
function compareTypeParity(appContent, consoleContent, violations) {
  ...
  if (!appTable) {
    pushViolation(violations, {
      ...
      message: 'app type contract missing search_events table block.',
    });
    return null;
  }
  if (!consoleTable) {
    pushViolation(violations, {
      ...
      message: 'console type contract missing search_events table block.',
    });
    return null;
  }
...
function validateReferenceBoundaries(violations) {
  ...
  for (const filePath of files) {
    ...
    if (!content.includes('search_events')) continue;
    const relFile = normalizePath(path.relative(CONSOLE_ROOT, filePath));
    if (!ALLOWED_SEARCH_EVENTS_REFERENCERS.has(relFile)) {
      pushViolation(violations, {
        rule: 'search_events_reference_outside_allowed_surfaces',
        file: relFile,
        line: 1,
        message: 'search_events references must stay within approved type/service surfaces.',
      });
    }
  }
}
...
function validateQueryColumnsAndMutations(allowedColumns, violations) {
  const fromPattern = String.raw`(?:['"]search_events['"]|TABLE_NAME|SEARCH_EVENTS_TABLE)`;
  ...
  for (const filePath of files) {
    ...
    while ((selectMatch = selectRegex.exec(content)) !== null) {
      ...
      if (invalidColumns.length > 0) {
        pushViolation(violations, {
          rule: 'non_schema_search_events_select_columns',
          file: relFile,
          line,
          message: `search_events select contains non-canonical columns: ${invalidColumns.join(', ')}`,
          snippet: selected,
        });
      }
    }
    ...
    while ((mutationMatch = mutationRegex.exec(content)) !== null) {
      ...
      if (!ALLOWED_MUTATION_OWNERS.has(relFile)) {
        pushViolation(violations, {
          rule: 'search_events_mutation_outside_allowed_services',
          file: relFile,
          line,
    ...
    if (ALLOWED_MUTATION_OWNERS.has(relFile) && /\bextra\s*:/.test(content)) {
      pushViolation(violations, {
        rule: 'non_schema_search_events_extra_write_key',
        file: relFile,
        line: 1,
        message: "search_events mutation payload must not write non-schema key 'extra'.",
      });
    }
  }
}
...
function validateAllowlist(name, expectedFields, serviceContent, violations) {
  ...
  if (!match) {
    pushViolation(violations, {
      ...
      file: 'src/services/searchEventsService.js',
      message: `search events service must declare ${name}.`,
    });
    return;
  }
...
function validateSearchEventsServiceContract(violations) {
  if (!fs.existsSync(CONSOLE_SEARCH_EVENTS_SERVICE_FILE)) {
    pushViolation(violations, {
      rule: 'search_events_service_missing',
      file: 'src/services/searchEventsService.js',
      message: 'searchEventsService.js not found.',
    });
    return;
  }

  const content = fs.readFileSync(CONSOLE_SEARCH_EVENTS_SERVICE_FILE, 'utf8');
  validateAllowlist('SEARCH_EVENT_CREATE_FIELDS', EXPECTED_CREATE_FIELDS, content, violations);

  const createFn = extractFunctionBlock(content, 'createSearchEvent');
  if (!createFn) {
    pushViolation(violations, {
      rule: 'create_search_event_missing',
      file: 'src/services/searchEventsService.js',
      message: 'createSearchEvent function is missing.',
    });
    return;
  }

  if (!/payload\.created_at\s*=\s*new Date\(\)\.toISOString\(\);/.test(createFn.block)) {
    pushViolation(violations, {
      rule: 'create_search_event_created_at_stamp_missing',
      file: 'src/services/searchEventsService.js',
      line: getLineNumber(content, createFn.start),
      message: 'createSearchEvent must stamp payload.created_at before insert.',
    });
  }
  if (!/payload\.source\s*=/.test(createFn.block) || !/['"]console['"]/.test(createFn.block)) {
    pushViolation(violations, {
      rule: 'create_search_event_source_default_missing',
      file: 'src/services/searchEventsService.js',
      line: getLineNumber(content, createFn.start),
      message: "createSearchEvent should default source to 'console'.",
    });
  }
  if (!/payload\.metadata\s*=\s*payload\.metadata\s*\?\?\s*input\?\.extra\s*\?\?\s*null;/.test(createFn.block)) {
    pushViolation(violations, {
      rule: 'create_search_event_metadata_fallback_missing',
      file: 'src/services/searchEventsService.js',
      line: getLineNumber(content, createFn.start),
      message: "createSearchEvent should map legacy input.extra into canonical payload.metadata fallback.",
    });
  }
}

function validateSearchAnalyticsServiceContract(violations) {
  if (!fs.existsSync(CONSOLE_SEARCH_ANALYTICS_SERVICE_FILE)) {
    pushViolation(violations, {
      rule: 'search_analytics_service_missing',
      file: 'src/services/searchAnalyticsService.js',
      message: 'searchAnalyticsService.js not found.',
    });
    return;
  }

  const content = fs.readFileSync(CONSOLE_SEARCH_ANALYTICS_SERVICE_FILE, 'utf8');
  if (!/from\('search_events'\)\.insert/.test(content)) {
    pushViolation(violations, {
      rule: 'search_analytics_track_insert_missing',
      file: 'src/services/searchAnalyticsService.js',
      message: "searchAnalyticsService.trackSearchEvent must insert into 'search_events'.",
    });
  }
  if (!/metadata\s*:/.test(content)) {
    pushViolation(violations, {
      rule: 'search_analytics_track_metadata_missing',
      file: 'src/services/searchAnalyticsService.js',
      message: "searchAnalyticsService.trackSearchEvent insert should include canonical 'metadata' field.",
    });
  }
}

function validateSearchServiceContract(violations) {
  if (!fs.existsSync(CONSOLE_SEARCH_SERVICE_FILE)) {
    pushViolation(violations, {
      rule: 'search_service_missing',
      file: 'src/services/searchService.js',
      message: 'searchService.js not found.',
    });
    return;
  }

  const content = fs.readFileSync(CONSOLE_SEARCH_SERVICE_FILE, 'utf8');
  if (!/const\s+SEARCH_EVENTS_TABLE\s*=\s*'search_events';/.test(content)) {
    pushViolation(violations, {
      rule: 'search_service_search_events_table_constant_missing',
      file: 'src/services/searchService.js',
      message: "searchService must declare SEARCH_EVENTS_TABLE = 'search_events'.",
    });
  }
  if (!/from\(SEARCH_EVENTS_TABLE\)\.insert/.test(content)) {
    pushViolation(violations, {
      rule: 'search_service_search_events_insert_constant_missing',
      file: 'src/services/searchService.js',
      message: 'searchService should use SEARCH_EVENTS_TABLE constant for search_events inserts.',
    });
  }
}
...
function run() {
  ...
  validateSearchEventsServiceContract(violations);
  validateSearchAnalyticsServiceContract(violations);
  validateSearchServiceContract(violations);

  const report = {
    generated_at: nowIso(),
    source: 'assert_search_events_surface_field_guard.js',
  ...
  if (violations.length > 0) {
    console.error('[search-events-surface-field-guard] FAIL: violations detected.');
    for (const violation of violations) {
      console.error(`- ${violation.file}:${violation.line} [${violation.rule}] ${violation.message}`);
    }
    console.error(`[search-events-surface-field-guard] Report written: ${OUT_FILE}`);
    process.exit(1);
  }

  console.log('[search-events-surface-field-guard] PASS: no search_events surface field violations detected.');
  console.log(`[search-events-surface-field-guard] Report written: ${OUT_FILE}`);
}
