...
const OUT_FILE = path.join(OUT_DIR, 'ivisit_main_wallet_surface_field_guard_report.json');

const TABLE_NAME = 'ivisit_main_wallet';
...
function fail(message) {
  console.error(`[ivisit-main-wallet-surface-field-guard] FAIL: ${message}`);
  process.exit(1);
}
...
function compareTypeParity(appContent, consoleContent, violations) {
  ...
  if (!appTable) {
    pushViolation(violations, {
      ...
      message: 'app type contract missing ivisit_main_wallet table block.',
    });
    return;
  }
  if (!consoleTable) {
    pushViolation(violations, {
      ...
      message: 'console type contract missing ivisit_main_wallet table block.',
    });
    return;
  }
...
function validateQueryColumnsAndMutations(violations) {
  const selectRegex =
    /from\(\s*['"]ivisit_main_wallet['"]\s*\)[\s\S]{0,260}?\.select\(\s*['"]([^'"]*)['"]\s*\)/g;
  const mutationRegex =
    /from\(\s*['"]ivisit_main_wallet['"]\s*\)[\s\S]{0,260}?\.(insert|update|upsert|delete)\s*\(/g;

  const files = listSourceFiles(CONSOLE_SRC);
  for (const filePath of files) {
    const content = fs.readFileSync(filePath, 'utf8');
    const relFile = path.relative(CONSOLE_ROOT, filePath);
    ...
    while ((selectMatch = selectRegex.exec(content)) !== null) {
      ...
      if (invalid.length > 0) {
        pushViolation(violations, {
          rule: 'non_schema_ivisit_main_wallet_select_columns',
          file: relFile,
          line,
          message: `ivisit_main_wallet select contains non-canonical columns: ${invalid.join(', ')}`,
          snippet: selected,
        });
      }
    }
    ...
    while ((mutationMatch = mutationRegex.exec(content)) !== null) {
      ...
      pushViolation(violations, {
        rule: 'direct_ivisit_main_wallet_mutation_forbidden',
        file: relFile,
        line,
        message: `direct console ivisit_main_wallet.${op} mutation is forbidden; use backend authority lanes.`,
      });
    }
  }
}
...
function run() {
  ...
  const report = {
    generated_at: nowIso(),
    source: 'assert_ivisit_main_wallet_surface_field_guard.js',
  ...
  if (violations.length > 0) {
    console.error('[ivisit-main-wallet-surface-field-guard] FAIL: violations detected.');
    for (const violation of violations) {
      console.error(`- ${violation.file}:${violation.line} [${violation.rule}] ${violation.message}`);
    }
    console.error(`[ivisit-main-wallet-surface-field-guard] Report written: ${OUT_FILE}`);
    process.exit(1);
  }

  console.log(
    '[ivisit-main-wallet-surface-field-guard] PASS: no ivisit_main_wallet surface field violations detected.'
  );
  console.log(`[ivisit-main-wallet-surface-field-guard] Report written: ${OUT_FILE}`);
}
