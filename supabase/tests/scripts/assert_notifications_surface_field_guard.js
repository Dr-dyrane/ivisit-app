...
const OUT_FILE = path.join(OUT_DIR, 'notifications_surface_field_guard_report.json');

const TABLE_NAME = 'notifications';
const REQUIRED_RELATIONSHIPS = ['notifications_user_id_fkey'];
...
function fail(message) {
  console.error(`[notifications-surface-field-guard] FAIL: ${message}`);
  process.exit(1);
}
...
function compareTypeParity(appContent, consoleContent, violations) {
  ...
  if (!appTable) {
    pushViolation(violations, {
      ...
      message: 'app type contract missing notifications table block.',
    });
    return null;
  }
  if (!consoleTable) {
    pushViolation(violations, {
      ...
      message: 'console type contract missing notifications table block.',
    });
    return null;
  }
...
function validateSelectColumns(allowedColumns, violations) {
  const selectRegex =
    /from\(\s*['"]notifications['"]\s*\)[\s\S]{0,420}?\.select\(\s*['"]([^'"]*)['"]\s*\)/g;
  ...
  for (const filePath of files) {
    ...
    while ((selectMatch = selectRegex.exec(content)) !== null) {
      ...
      if (invalidColumns.length > 0) {
        pushViolation(violations, {
          rule: 'non_schema_notifications_select_columns',
          file: relFile,
          line,
          message: `notifications select contains non-canonical columns: ${invalidColumns.join(', ')}`,
          snippet: selected,
        });
      }
    }
...
function run() {
  ...
  const report = {
    generated_at: nowIso(),
    source: 'assert_notifications_surface_field_guard.js',
  ...
  if (violations.length > 0) {
    console.error('[notifications-surface-field-guard] FAIL: violations detected.');
    for (const violation of violations) {
      console.error(`- ${violation.file}:${violation.line} [${violation.rule}] ${violation.message}`);
    }
    console.error(`[notifications-surface-field-guard] Report written: ${OUT_FILE}`);
    process.exit(1);
  }

  console.log(
    '[notifications-surface-field-guard] PASS: no notifications surface field violations detected.'
  );
  console.log(`[notifications-surface-field-guard] Report written: ${OUT_FILE}`);
}
