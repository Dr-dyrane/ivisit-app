...
const OUT_FILE = path.join(OUT_DIR, 'emergency_requests_surface_field_guard_report.json');
...
const LEGACY_ALIAS_RULES = [
  {
    ...
    message:
      'Legacy payment_method_id alias must stay in emergency compatibility boundaries only.',
  },
  {
    alias: 'estimated_arrival',
    pattern: /\bestimated_arrival\b/g,
    allowedIn: new Set(['src/utils/emergencyRequestMapper.js']),
    message: 'Legacy estimated_arrival alias must stay in emergency mapper compatibility boundary only.',
  },
  {
    alias: 'next_estimated_arrival',
    pattern: /\bnext_estimated_arrival\b/g,
    allowedIn: new Set(['src/utils/emergencyRequestMapper.js']),
    message:
      'Legacy next_estimated_arrival alias must stay in emergency mapper compatibility boundary only.',
  },
  {
    alias: 'bed_type',
    pattern: /\bbed_type\b/g,
    allowedIn: new Set([
      'src/services/emergencyResponseService.js',
      'src/utils/emergencyRequestMapper.js',
    ]),
    message: 'Legacy bed_type alias must stay in emergency compatibility boundaries only.',
  },
];
...
function fail(message) {
  console.error(`[emergency-requests-surface-field-guard] FAIL: ${message}`);
  process.exit(1);
}
...
function isLegacyAliasScanFile(relPath) {
  if (!relPath.startsWith('src/')) return false;
  if (relPath.startsWith('src/types/')) return false;
  return /emergency|Emergency|HospitalModal/.test(relPath);
}
...
function compareTypeParity(appContent, consoleContent, violations) {
  ...
  if (!appTable) {
    pushViolation(violations, {
      ...
      message: 'app type contract missing emergency_requests table block.',
    });
    return null;
  }
  if (!consoleTable) {
    pushViolation(violations, {
      ...
      message: 'console type contract missing emergency_requests table block.',
    });
    return null;
  }
...
function validateQueryColumnsAndMutations(allowedColumns, violations) {
  const selectRegex =
    /from\(\s*['"]emergency_requests['"]\s*\)[\s\S]{0,420}?\.select\(\s*['"]([^'"]*)['"]\s*\)/g;
  const mutationRegex =
    /from\(\s*['"]emergency_requests['"]\s*\)[\s\S]{0,260}?\.(insert|update|upsert|delete)\s*\(/g;
  ...
  for (const filePath of files) {
    ...
    while ((selectMatch = selectRegex.exec(content)) !== null) {
      ...
      if (invalidColumns.length > 0) {
        pushViolation(violations, {
          rule: 'non_schema_emergency_requests_select_columns',
          file: relFile,
          line,
          message: `emergency_requests select contains non-canonical columns: ${invalidColumns.join(', ')}`,
          snippet: selected,
        });
      }
    }
    ...
    while ((mutationMatch = mutationRegex.exec(content)) !== null) {
      ...
      pushViolation(violations, {
        rule: 'direct_emergency_requests_mutation_forbidden',
        file: relFile,
        line,
        message: `direct console emergency_requests.${op} mutation is forbidden; use backend RPC authority lanes.`,
      });
    }
  }
}

function validateLegacyAliasBoundaries(violations) {
  const files = listSourceFiles(CONSOLE_SRC);
  for (const filePath of files) {
    const relFile = normalizePath(path.relative(CONSOLE_ROOT, filePath));
    if (!isLegacyAliasScanFile(relFile)) continue;
    ...
    for (const rule of LEGACY_ALIAS_RULES) {
      ...
      while ((match = regex.exec(content)) !== null) {
        if (rule.allowedIn.has(relFile)) continue;
        pushViolation(violations, {
          rule: `legacy_alias_${rule.alias}_outside_boundary`,
          file: relFile,
...
function run() {
  ...
  validateLegacyAliasBoundaries(violations);

  const report = {
    generated_at: nowIso(),
    source: 'assert_emergency_requests_surface_field_guard.js',
  ...
  if (violations.length > 0) {
    console.error('[emergency-requests-surface-field-guard] FAIL: violations detected.');
    for (const violation of violations) {
      console.error(`- ${violation.file}:${violation.line} [${violation.rule}] ${violation.message}`);
    }
    console.error(`[emergency-requests-surface-field-guard] Report written: ${OUT_FILE}`);
    process.exit(1);
  }

  console.log(
    '[emergency-requests-surface-field-guard] PASS: no emergency_requests surface field violations detected.'
  );
  console.log(`[emergency-requests-surface-field-guard] Report written: ${OUT_FILE}`);
}
