...
const OUT_FILE = path.join(OUT_DIR, 'user_activity_surface_field_guard_report.json');

const TABLE_NAME = 'user_activity';
const REQUIRED_RELATIONSHIPS = ['user_activity_user_id_fkey'];
...
function fail(message) {
  console.error(`[user-activity-surface-field-guard] FAIL: ${message}`);
  process.exit(1);
}
...
function compareTypeParity(appContent, consoleContent, violations) {
  ...
  if (!appTable) {
    pushViolation(violations, {
      ...
      message: 'app type contract missing user_activity table block.',
    });
    return null;
  }
  if (!consoleTable) {
    pushViolation(violations, {
      ...
      message: 'console type contract missing user_activity table block.',
    });
    return null;
  }
...
function validateQueryColumnsAndMutations(allowedColumns, violations) {
  const selectRegex =
    /from\(\s*['"]user_activity['"]\s*\)[\s\S]{0,420}?\.select\(\s*['"]([^'"]*)['"]\s*\)/g;
  const mutationRegex =
    /from\(\s*['"]user_activity['"]\s*\)[\s\S]{0,260}?\.(insert|update|upsert|delete)\s*\(/g;
  ...
  for (const filePath of files) {
    ...
    while ((selectMatch = selectRegex.exec(content)) !== null) {
      ...
      if (invalidColumns.length > 0) {
        pushViolation(violations, {
          rule: 'non_schema_user_activity_select_columns',
          file: relFile,
          line,
          message: `user_activity select contains non-canonical columns: ${invalidColumns.join(', ')}`,
          snippet: selected,
        });
      }
    }
    ...
    while ((mutationMatch = mutationRegex.exec(content)) !== null) {
      ...
      pushViolation(violations, {
        rule: 'direct_user_activity_mutation_forbidden',
        file: relFile,
        line,
        message: `direct console user_activity.${op} mutation is forbidden; use log_user_activity RPC or backend automation.`,
      });
    }
  }
}
...
function run() {
  ...
  const report = {
    generated_at: nowIso(),
    source: 'assert_user_activity_surface_field_guard.js',
  ...
  if (violations.length > 0) {
    console.error('[user-activity-surface-field-guard] FAIL: violations detected.');
    for (const violation of violations) {
      console.error(`- ${violation.file}:${violation.line} [${violation.rule}] ${violation.message}`);
    }
    console.error(`[user-activity-surface-field-guard] Report written: ${OUT_FILE}`);
    process.exit(1);
  }

  console.log(
    '[user-activity-surface-field-guard] PASS: no user_activity surface field violations detected.'
  );
  console.log(`[user-activity-surface-field-guard] Report written: ${OUT_FILE}`);
}
